import DomainSemantics.Typing
import DomainSemantics.Variable!
...
theorem HasType.sort : HasType env U Γ (.sort l) (.sort true) := .sortDF
...
theorem HasType.forallE
    ...
    HasType env U Γ (.forallE A body) (.sort v) := .forallEDF h1 h2
...
theorem IsDefEqCtx.closed (H : CtxClosed Γ₀) :
  ...
  | .succ h1 h2 =>
    ...
    ⟨⟨c1, h2.closedN c1⟩, ⟨c2, by simpa [h1.length_eq] using h2.symm.closedN c1⟩⟩

theorem IsDefEq.mono {env env' : VEnv} (_henv : env ≤ env')
...
theorem CtxWF.closed (h : OnCtx Γ (IsType env U)) : CtxClosed Γ :=
  match Γ, h with
  | [], _ => trivial
  | _::_, ⟨h1, _, h2⟩ => ⟨closed h1, h2.closedN (closed h1)⟩
...
theorem HasType.weakN (W : Ctx.LiftN n k Γ Γ') (H : env.HasType U Γ e A) :
    env.HasType U Γ' (e.liftN n k) (A.liftN n k) := IsDefEq.weakN W H

variable {env : VEnv} in
theorem IsType.weakN (W : Ctx.LiftN n k Γ Γ') (H : env.IsType U Γ A) :
    env.IsType U Γ' (A.liftN n k) := let ⟨_, h⟩ := H; ⟨_, h.weakN W⟩

variable {env : VEnv} in
theorem IsDefEq.weak (H : env.IsDefEq U Γ e1 e2 A) :
    env.IsDefEq U (B::Γ) e1.lift e2.lift A.lift := H.weakN .one

variable {env : VEnv} in
theorem IsDefEqU.weak (H : env.IsDefEqU U Γ e1 e2) :
    env.IsDefEqU U (B::Γ) e1.lift e2.lift := let ⟨_, H⟩ := H; ⟨_, H.weak⟩

variable {env : VEnv} in
theorem IsDefEqU.weakN (W : Ctx.LiftN n k Γ Γ') (H : env.IsDefEqU U Γ e1 e2) :
    env.IsDefEqU U Γ' (e1.liftN n k) (e2.liftN n k) := let ⟨_, H⟩ := H; ⟨_, H.weakN W⟩

variable {env : VEnv} in
theorem IsDefEq.weakR (hΓ : CtxClosed Γ) (H : env.IsDefEq U Γ e1 e2 A) (Γ') :
    env.IsDefEq U (Γ ++ Γ') e1 e2 A := by
  have ⟨h1, h2, h3⟩ := H.closedN' hΓ
  simpa [h1.liftN_eq, h2.liftN_eq, h3.liftN_eq] using H.weakN (.right hΓ Γ')
...
variable {env : VEnv} in
nonrec theorem HasType.weak0 (H : env.HasType U [] e A) : env.HasType U Γ e A := H.weak0
...
theorem IsDefEq.weak' (W : Ctx.Lift' l Γ Γ') (H : env.IsDefEq U Γ e1 e2 A) :
  ...
  | succ n ih =>
    ...
    exact (ih W1 Lift.depth_consN).weakN W2

variable {env : VEnv} in
theorem HasType.weak' (W : Ctx.Lift' l Γ Γ') (H : env.HasType U Γ e A) :
    env.HasType U Γ' (e.lift' l) (A.lift' l) := IsDefEq.weak' W H

variable {env : VEnv} in
theorem IsType.weak' (W : Ctx.Lift' l Γ Γ') (H : env.IsType U Γ A) :
    env.IsType U Γ' (A.lift' l) := let ⟨_, h⟩ := H; ⟨_, h.weak' W⟩

variable {env : VEnv} in
theorem IsDefEqU.weak' (W : Ctx.Lift' n Γ Γ') (H : env.IsDefEqU U Γ e1 e2) :
    env.IsDefEqU U Γ' (e1.lift' n) (e2.lift' n) := let ⟨_, H⟩ := H; ⟨_, H.weak' W⟩

theorem IsType.lookup (h : OnCtx Γ (IsType env U)) (hL : Lookup Γ n A) :
    env.IsType U Γ A := h.lookup hL <| .weakN .one

variable! {env : VEnv} (h₀ : env.HasType U Γ₀ e₀ A₀) in
...
theorem HasType.instN {env : VEnv} (W : Ctx.InstN Γ₀ e₀ A₀ k Γ₁ Γ)
    (H : env.HasType U Γ₁ e A) (h₀ : env.HasType U Γ₀ e₀ A₀) :
    env.HasType U Γ (e.inst e₀ k) (A.inst e₀ k) := IsDefEq.instN h₀ W H

theorem IsType.instN {env : VEnv} (W : Ctx.InstN Γ₀ e₀ A₀ k Γ₁ Γ)
    (H : env.IsType U Γ₁ A) (h₀ : env.HasType U Γ₀ e₀ A₀) :
    env.IsType U Γ (A.inst e₀ k) := let ⟨_, h⟩ := H; ⟨_, h.instN W h₀⟩

theorem IsDefEqU.instN {env : VEnv} (W : Ctx.InstN Γ₀ e₀ A₀ k Γ₁ Γ)
    (H : env.IsDefEqU U Γ₁ e1 e2) (h₀ : env.HasType U Γ₀ e₀ A₀) :
    env.IsDefEqU U Γ (e1.inst e₀ k) (e2.inst e₀ k) := let ⟨_, h⟩ := H; ⟨_, h.instN h₀ W⟩

theorem _root_.DomainSemantics.Ctx.InstN.wf {env : VEnv} (W : Ctx.InstN Γ₀ e₀ A₀ k Γ₁ Γ)
    (h₀ : env.HasType U Γ₀ e₀ A₀) (H : OnCtx Γ₁ (env.IsType U)) :
    OnCtx Γ₀ (env.IsType U) ∧ OnCtx Γ (env.IsType U) := by
  induction W with
  | zero => exact ⟨H.1, H.1⟩
  | succ W ih => let ⟨h1, h2⟩ := ih H.1; exact ⟨h1, h2, .instN W H.2 h₀⟩

theorem IsDefEq.defeqDF_l' {env : VEnv} (h1 : env.IsDefEq U Γ A A' (.sort u))
  ...
  simpa [instN_bvar0] using
    instN (h1.weakN (.one (A := A')) |>.symm.defeq (.bvar .zero)) H2 (.weakN H1 h2)

theorem IsDefEq.defeqDF_l {env : VEnv} (h1 : env.IsDefEq U Γ A A' (.sort u))
    (h2 : env.IsDefEq U (A::Γ) e1 e2 B) : env.IsDefEq U (A'::Γ) e1 e2 B :=
  .defeqDF_l' (Δ := []) h1 h2

theorem HasType.defeq_l {env : VEnv} (h1 : env.IsDefEq U Γ A A' (.sort u))
    (h2 : env.HasType U (A::Γ) e B) : env.HasType U (A'::Γ) e B := h1.defeqDF_l h2

theorem IsDefEq.defeqDFC' {env : VEnv} (h1 : IsDefEqCtx env U Γ₀ Γ₁ Γ₂)
...
theorem IsDefEq.defeqDFC {env : VEnv} (h1 : IsDefEqCtx env U Γ₀ Γ₁ Γ₂)
    (h2 : env.IsDefEq U Γ₁ e₁ e₂ A) : env.IsDefEq U Γ₂ e₁ e₂ A := .defeqDFC' (Δ := []) h1 h2

theorem HasType.defeqDFC {env : VEnv} (h1 : IsDefEqCtx env U Γ₀ Γ₁ Γ₂)
    (h2 : env.HasType U Γ₁ e A) : env.HasType U Γ₂ e A := IsDefEq.defeqDFC h1 h2

theorem IsType.defeqDFC {env : VEnv} (h1 : IsDefEqCtx env U Γ₀ Γ₁ Γ₂)
    (h2 : env.IsType U Γ₁ A) : env.IsType U Γ₂ A := h2.imp fun _ => (·.defeqDFC h1)

theorem IsDefEqU.defeqDFC {env : VEnv} (h1 : IsDefEqCtx env U Γ₀ Γ₁ Γ₂)
    (h2 : env.IsDefEqU U Γ₁ e₁ e₂) : env.IsDefEqU U Γ₂ e₁ e₂ :=
  let ⟨_, h2⟩ := h2; ⟨_, h2.defeqDFC h1⟩

theorem IsDefEqCtx.symm {env : VEnv} :
    IsDefEqCtx env U Γ₀ Γ₁ Γ₂ → IsDefEqCtx env U Γ₀ Γ₂ Γ₁
  | .zero => .zero
  | .succ h1 h2 => .succ (h1.symm) (h2.symm.defeqDFC h1)
...
theorem IsDefEq.forallE_inv'
  ...
  | forallEDF h1 h2 =>
    ...
    · exact ⟨⟨_, h1.hasType.2⟩, _, h2.hasType.2.defeq_l h1⟩
  ...
  | @beta _ _ e _ _ h1 h2 ih1 ih2 =>
    ...
    | forallE A B =>
      ...
      exact ⟨A1.instN .zero h2, A2.instN (.succ .zero) h2⟩
    | _ => cases eq
  | eta _ ih =>
    obtain ⟨⟨⟩⟩ | eq := eq
    exact ih (.inl eq)
...
variable! {env : VEnv} in
theorem HasType.forallE_inv (H : env.HasType U Γ (A.forallE B) V) :
    env.IsType U Γ A ∧ env.IsType U (A::Γ) B := H.forallE_inv' (.inl rfl)

variable! {env : VEnv} in
theorem IsType.forallE_inv (H : env.IsType U Γ (A.forallE B)) :
    env.IsType U Γ A ∧ env.IsType U (A::Γ) B := let ⟨_, h⟩ := H; h.forallE_inv

variable! {env : VEnv} in
theorem IsDefEq.isType' (hΓ : OnCtx Γ (env.IsType U)) (H : env.IsDefEq U Γ e1 e2 A) :
    env.IsType U Γ A := by
  induction H with
  | bvar h => exact .lookup hΓ h
  | proofIrrel h1 => exact ⟨_, h1⟩
  | sortDF => exact ⟨_, .sort⟩
  | symm _ ih => exact ih hΓ
  | trans _ _ ih1 => exact ih1 hΓ
  | appDF _ h2 ih1 => exact (ih1 hΓ).forallE_inv.2.instN .zero h2.hasType.1
  | lamDF h1 _ _ ih2 =>
    let ⟨_, h⟩ := ih2 ⟨hΓ, _, h1.hasType.1⟩
    exact ⟨_, .forallE h1.hasType.1 h⟩
  | forallEDF h1 _ ih1 ih2 => exact ⟨_, .sort⟩
  | defeqDF h1 => exact ⟨_, h1.hasType.2⟩
  | beta _ h2 ih1 ih2 =>
    have ⟨_, h⟩ := ih2 hΓ
    exact (ih1 ⟨hΓ, _, h.hasType.2⟩).instN .zero h2
  | eta _ ih => exact ih hΓ

variable {env : VEnv} in
theorem IsDefEq.isType (hΓ : OnCtx Γ (env.IsType U))
    (H : env.IsDefEq U Γ e1 e2 A) : env.IsType U Γ A := H.isType' hΓ
...
theorem IsDefEq.instDF
  ...
  have ⟨_, hB⟩ := hf.isType (Γ := _::_) ⟨hΓ, _, hA⟩
  have H2 {f f' B v}
