...
local notation:65 Γ " ⊢ " e " : " A:30 => IsDefEq env uvars Γ e e A
local notation:65 Γ " ⊢ " e1 " ≡ " e2 " : " A:30 => IsDefEq env uvars Γ e1 e2 A

inductive IsDefEq (env : VEnv) (uvars : Nat) : List VExpr → VExpr → VExpr → VExpr → Prop where
