...
`Shape` with Π/Σ/Id-types, λ/pair/refl-values, and the natural numbers. Π and Σ
store a domain shape plus a finite graph (`List (Shape × Shape)`) encoding
the function/family on the second component. `lam` is a function value
carried by its graph; `pair` is a value carrying its two component shapes
directly. `nat` is the nullary type shape for natural numbers, `zero` is
the nullary value shape for `0`, and `succ` carries the predecessor's
shape. `id` stores the three shape arguments of the identity type; `refl`
is a nullary value shape — all canonical identity proofs share it. -/
inductive ShapeS (Shape : Type) : Type where
  | bot : ShapeS Shape
  | sort (rel : Bool) : ShapeS Shape
  | unit (rel : Bool) : ShapeS Shape
  | forallE : Shape → List (Shape × Shape) → ShapeS Shape
  | lam : List (Shape × Shape) → ShapeS Shape
  | sigma : Shape → List (Shape × Shape) → ShapeS Shape
  | pair : Shape → Shape → ShapeS Shape
  | nat : ShapeS Shape
  | zero : ShapeS Shape
  | succ (n : Shape) : ShapeS Shape
  | id : Shape → Shape → Shape → ShapeS Shape
  | refl : Shape → ShapeS Shape
...
@[match_pattern] abbrev Shape.nat {n} : Shape (n+1) := ShapeS.nat
@[match_pattern] abbrev Shape.zero {n} : Shape (n+1) := ShapeS.zero
@[match_pattern] abbrev Shape.succ {n} (v : Shape n) : Shape (n+1) := ShapeS.succ v
@[match_pattern] def Shape.refl {n} (w : Shape n) : Shape (n+1) := ShapeS.refl w
...
def Shape.Compat : ∀ {n}, Shape n → Shape n → Bool
  ...
  | _+1, .unit r, .unit r' => r = r'
  | _+1, .forallE s f, .forallE s' f' => s.Compat s' && ShapeFun.Compat Compat f f'
  | _+1, .lam f, .lam f' => ShapeFun.Compat Compat f f'
  | _+1, .sigma s f, .sigma s' f' => s.Compat s' && ShapeFun.Compat Compat f f'
  | _+1, .pair a b, .pair a' b' => a.Compat a' && b.Compat b'
  | _+1, .nat, .nat => true
  | _+1, .zero, .zero => true
  | _+1, .succ v, .succ v' => v.Compat v'
  | _+1, .id A a b, .id A' a' b' => A.Compat A' && a.Compat a' && b.Compat b'
  | _+1, .refl w, .refl w' => w.Compat w'
  | _, _, _ => false
...
def Shape.ble : ∀ {n}, Shape n → Shape n → Bool
  ...
  | _+1, .sigma s f, .sigma s' f' => s.ble s' && ShapeFun.ble ble f f'
  | _+1, .pair a b, .pair a' b' => a.ble a' && b.ble b'
  | _+1, .nat, .nat => true
  | _+1, .zero, .zero => true
  | _+1, .succ v, .succ v' => v.ble v'
  | _+1, .id A a b, .id A' a' b' => A.ble A' && a.ble a' && b.ble b'
  | _+1, .refl w, .refl w' => w.ble w'
  | _, _, _ => false
...
theorem Shape.LE.def {s s' : Shape (n + 1)} : s ≤ s' ↔
    ...
    | .sigma s f, .sigma s' f' => s ≤ s' ∧ ShapeFun.LE f f'
    | .pair a b, .pair a' b' => a ≤ a' ∧ b ≤ b'
    | .nat, .nat => True
    | .zero, .zero => True
    | .succ v, .succ v' => v ≤ v'
    | .id A a b, .id A' a' b' => A ≤ A' ∧ a ≤ a' ∧ b ≤ b'
    | .refl w, .refl w' => w ≤ w'
    | _, _ => False := by
...
theorem Shape.le_unit {s : Shape (n+1)} : s ≤ .unit r ↔ s = .bot ∨ s = .unit r := by
  cases n <;> simp [bot, (· ≤ ·), Shape.LE] <;> cases s <;>
    simp [ble] <;> exact ⟨fun h => h ▸ rfl, fun h => by injection h⟩

theorem Shape.unit_le {s : Shape (n+1)} : .unit r ≤ s ↔ .unit r = s := by
  cases n <;> simp [(· ≤ ·), Shape.LE] <;> cases s <;> simp [ble, Shape]
...
theorem Shape.sigma_le {s : Shape (n+1)} :
    .sigma a b ≤ s ↔ ∃ a' b', a ≤ a' ∧ ShapeFun.LE b b' ∧ .sigma a' b' = s := by
  rw [Shape.LE.def]; cases s <;> simp [Shape]

theorem Shape.pair_le {s : Shape (n+1)} :
    .pair x y ≤ s ↔ ∃ x' y', x ≤ x' ∧ y ≤ y' ∧ .pair x' y' = s := by
  rw [Shape.LE.def]; cases s <;> simp [Shape]

theorem Shape.nat_le {s : Shape (n+1)} : .nat ≤ s ↔ s = .nat := by
  rw [Shape.LE.def]; cases s <;> simp [Shape, Shape.nat]

theorem Shape.zero_le {s : Shape (n+1)} : .zero ≤ s ↔ s = .zero := by
  rw [Shape.LE.def]; cases s <;> simp [Shape, Shape.zero]

theorem Shape.succ_le {s : Shape (n+1)} {v : Shape n} :
    .succ v ≤ s ↔ ∃ v', v ≤ v' ∧ .succ v' = s := by
  rw [Shape.LE.def]; cases s <;> simp [Shape, Shape.succ]

@[simp] theorem Shape.succ_le_succ {v v' : Shape n} : v.succ ≤ v'.succ ↔ v ≤ v' := by
  refine Shape.succ_le.trans ⟨?_, fun h => ⟨_, h, rfl⟩⟩
  rintro ⟨_, h, ⟨⟩⟩; exact h

theorem Shape.id_le {s : Shape (n+1)} :
    .id A a b ≤ s ↔ ∃ A' a' b', A ≤ A' ∧ a ≤ a' ∧ b ≤ b' ∧ .id A' a' b' = s := by
  rw [Shape.LE.def]; cases s <;> simp [Shape]
  refine ⟨fun ⟨h1, h2, h3⟩ => ⟨_, h1, _, h2, _, h3, rfl, rfl, rfl⟩, ?_⟩
  rintro ⟨_, h1, _, h2, _, h3, rfl, rfl, rfl⟩; exact ⟨h1, h2, h3⟩

theorem Shape.refl_le {w : Shape n} {s : Shape (n+1)} :
    (Shape.refl w : Shape (n+1)) ≤ s ↔ ∃ w', s = Shape.refl w' ∧ w ≤ w' := by
  rw [Shape.LE.def]; cases s <;> simp [Shape, Shape.refl]
...
theorem Shape.Compat.mono_r {n} {s t t' : Shape n}
  ...
  induction n with
  | zero =>
    cases s <;> cases t <;> cases t' <;>
      simp_all [Compat, (·≤·), Shape.LE, Shape.ble]
  | succ n ih
  ...
  · exact go le H
  · exact ⟨ih le.1 H.1, go le.2 H.2⟩
  · exact ⟨ih le.1 H.1, ih le.2 H.2⟩
  · exact ih le H
  · exact ⟨⟨ih le.1 H.1.1, ih le.2.1 H.1.2⟩, ih le.2.2 H.2⟩
  · exact ih le H
...
def Shape.lift : ∀ {n} m, Shape n → Shape m
  ...
  | _+1, _+1, .unit r => .unit r
  | _+1, _+1, .forallE s f => .forallE (lift _ s) <| ShapeFun.lift (lift _) f
  | _+1, _+1, .lam f => .lam <| ShapeFun.lift (lift _) f
  | _+1, _+1, .sigma s f => .sigma (lift _ s) <| ShapeFun.lift (lift _) f
  | _+1, _+1, .pair a b => .pair (lift _ a) (lift _ b)
  | _+1, _+1, .nat => .nat
  | _+1, _+1, .zero => .zero
  | _+1, _+1, .succ v => .succ (lift _ v)
  | _+1, _+1, .id A a b => .id (lift _ A) (lift _ a) (lift _ b)
  | _+1, _+1, .refl w => .refl (lift _ w)
...
def Shape.olift : ∀ {n m}, Shape n → Option (Shape m)
  ...
  | _+1, 0, _ => none
  | _+1, _+1, .unit r => some (.unit r)
  | _+1, _+1, .id A a b => return .id (← A.olift) (← a.olift) (← b.olift)
  | _+1, _+1, .refl w => return .refl (← w.olift)
  | _+1, _+1, .forallE s f => return .forallE (← s.olift) (← ShapeFun.olift olift f)
  | _+1, _+1, .lam f => return .lam (← ShapeFun.olift olift f)
  | _+1, _+1, .sigma s f => return .sigma (← s.olift) (← ShapeFun.olift olift f)
  | _+1, _+1, .pair a b => return .pair (← a.olift) (← b.olift)
  | _+1, _+1, .nat => return .nat
  | _+1, _+1, .zero => return .zero
  | _+1, _+1, .succ v => return .succ (← v.olift)
...
theorem Shape.olift_thm (le : n ≤ m) {s : Shape m} {t : Shape n} :
  ...
  unfold olift; split
    ...
    <;> cases t <;> simp [lift, bot, sort, refl]
  iterate 4 · grind
  all_goals have le := Nat.le_of_succ_le_succ le
  all_goals simp -failIfUnchanged [olift_thm le, go olift_thm le]; grind
...
def Shape.join : ∀ {n}, Shape n → Shape n → Shape n
  ...
  | _+1, .unit r, .unit r' => if r = r' then .unit r else .bot
  | _+1, .forallE s f, .forallE s' f' => .forallE (join s s') (ShapeFun.join join f f')
  | _+1, .lam f, .lam f' => .lam (ShapeFun.join join f f')
  | _+1, .sigma s f, .sigma s' f' => .sigma (join s s') (ShapeFun.join join f f')
  | _+1, .pair a b, .pair a' b' => .pair (join a a') (join b b')
  | _+1, .nat, .nat => .nat
  | _+1, .zero, .zero => .zero
  | _+1, .succ v, .succ v' => .succ (join v v')
  | _+1, .id A a b, .id A' a' b' => .id (join A A') (join a a') (join b b')
  | _+1, .refl w, .refl w' => .refl (join w w')
  | _+1, _, _ => .bot
...
theorem Shape.lift_join {x y : Shape n} (le : n ≤ m) :
  ...
  cases x with cases y <;> simp [join, lift, go, sort, refl, ih]
  | sort => split <;> simp [lift, sort]
  | unit => split <;> simp [lift]
...
def Shape.WF : ∀ {n}, Shape n → Prop
  | 0, _ | _+1, .bot | _+1, .sort .. | _+1, .unit .. => True
  | _+1, .forallE s f => s.WF ∧ ShapeFun.WF WF f
  | _+1, .lam f => ShapeFun.WF WF f ∧ ShapeFun.NonZero f
  | _+1, .sigma s f => s.WF ∧ ShapeFun.WF WF f
  | _+1, .pair a b => a.WF ∧ b.WF ∧ (¬a ≤ .bot ∨ ¬b ≤ .bot)
  | _+1, .nat => True
  | _+1, .zero => True
  | _+1, .succ v => v.WF
  | _+1, .id A a b => A.WF ∧ a.WF ∧ b.WF
  | _+1, .refl w => w.WF
...
theorem Shape.WF.lift_iff (le : n ≤ m) : WF (x.lift m) ↔ WF (n := n) x := by
  ...
  | pair => intros; apply or_congr <;> exact not_congr ((lift_le_bot le).trans le_bot.symm)
...
def WShape.unit (r : Bool) : WShape (n+1) := ⟨.unit r, trivial⟩
protected def WShape.id (A a b : WShape n) : WShape (n + 1) := ⟨.id A.1 a.1 b.1, A.2, a.2, b.2⟩
def WShape.refl (w : WShape n) : WShape (n + 1) := ⟨.refl w.1, w.2⟩
...
/-- Σ-type shape: domain shape + finite graph of fst-value → snd-type. -/
def WShape.sigma (s : WShape n) (f : WShapeFun n) : WShape (n + 1) := ⟨.sigma s.1 f.1, s.2, f.2⟩

/-- Pair value shape: requires at least one component non-bot (else collapse to `.bot`). -/
def WShape.pair (a b : WShape n) (h : ¬a.1 ≤ .bot ∨ ¬b.1 ≤ .bot) :
    WShape (n + 1) := ⟨.pair a.1 b.1, a.2, b.2, h⟩

/-- Smart constructor for `WShape.pair`: collapses to `.bot` when both
components are `.bot`. Mirrors `WShape.lam'`. -/
def WShape.pair' (a b : WShape n) : WShape (n + 1) :=
  if h : ¬a.1 ≤ .bot ∨ ¬b.1 ≤ .bot then .pair a b h else .bot

theorem WShape.pair_eq_pair' {a b : WShape n} {h} : WShape.pair a b h = .pair' a b := by
  simp [pair', h]

protected def WShape.nat : WShape (n + 1) := ⟨.nat, trivial⟩
protected def WShape.zero : WShape (n + 1) := ⟨.zero, trivial⟩
protected def WShape.succ (v : WShape n) : WShape (n + 1) := ⟨.succ v.1, v.2⟩
...
@[simp] theorem WShape.pair'_bot_bot : WShape.pair' (n := n) .bot .bot = .bot := by
  simp [pair', bot]
...
def WShape.casesOn' {motive : WShape (n+1) → Sort u}
    ...
    (unit : ∀ r, motive (.unit r))
    (forallE : ∀ s f, motive (.forallE s f))
    (lam : ∀ f h, motive (.lam f h))
    (sigma : ∀ s f, motive (.sigma s f))
    (pair : ∀ a b h, motive (.pair a b h))
    (nat : motive .nat)
    (zero : motive .zero)
    (succ : ∀ v, motive (.succ v))
    (id : ∀ A a b, motive (.id A a b))
    (refl : ∀ w, motive (.refl w)) : motive s := by
  obtain ⟨s, wf⟩ := s
  cases s with
  | bot => exact bot
  | sort r => exact sort r
  ...
  | sigma s' f' => exact sigma ⟨s', wf.1⟩ ⟨f', wf.2⟩
  | pair a' b' => exact pair ⟨a', wf.1⟩ ⟨b', wf.2.1⟩ wf.2.2
  | nat => exact nat
  | zero => exact zero
  | succ v' => exact succ ⟨v', wf⟩
  | id A' a' b' => exact id ⟨A', wf.1⟩ ⟨a', wf.2.1⟩ ⟨b', wf.2.2⟩
  | refl w' => exact refl ⟨w', wf⟩
...
def WShape.casesOn {motive : ∀ {n}, WShape n → Sort u}
    ...
    (unit : ∀ {n'} r, motive (n := n'+1) (.unit r))
    (forallE : ∀ {n'} s f, motive (n := n'+1) (.forallE s f))
    (lam : ∀ {n'} f h, motive (n := n'+1) (.lam f h))
    (sigma : ∀ {n'} s f, motive (n := n'+1) (.sigma s f))
    (pair : ∀ {n'} a b h, motive (n := n'+1) (.pair a b h))
    (nat : ∀ {n'}, motive (n := n'+1) .nat)
    (zero : ∀ {n'}, motive (n := n'+1) .zero)
    (succ : ∀ {n'} v, motive (n := n'+1) (.succ v))
    (id : ∀ {n'} A a b, motive (n := n'+1) (.id A a b))
    (refl : ∀ {n'} w, motive (n := n'+1) (.refl w)) : motive s := by
  cases n with
  | zero =>
    obtain ⟨s, wf⟩ := s
    cases s with
    | bot => exact bot
    | sort r => exact sort r
  | succ n => exact s.casesOn' bot sort unit forallE lam sigma pair nat zero succ id refl
...
@[simp] theorem WShape.lift_unit : (WShape.unit r : WShape (n+1)).lift (m+1) = .unit r := by
  ext; simp [lift, unit, Shape.olift]
...
@[simp] theorem WShape.lift_sigma {s : WShape n} {f : WShapeFun n} (h : n ≤ m) :
    (WShape.sigma s f).lift (m+1) = .sigma (s.lift m) (f.lift m) := by
  ext; simp [lift_val (Nat.succ_le_succ h), Shape.lift, WShape.sigma,
    lift_val h, WShapeFun.lift_val h]

@[simp] theorem WShape.lift_nat (h : n ≤ m) :
    (WShape.nat : WShape (n+1)).lift (m+1) = WShape.nat := by
  ext; simp [lift_val (Nat.succ_le_succ h), Shape.lift, WShape.nat, Shape.nat]

@[simp] theorem WShape.lift_zero (h : n ≤ m) :
    (WShape.zero : WShape (n+1)).lift (m+1) = WShape.zero := by
  ext; simp [lift_val (Nat.succ_le_succ h), Shape.lift, WShape.zero, Shape.zero]

@[simp] theorem WShape.lift_succ {v : WShape n} (h : n ≤ m) :
    (WShape.succ v).lift (m+1) = .succ (v.lift m) := by
  ext; simp [lift_val (Nat.succ_le_succ h), Shape.lift, WShape.succ, Shape.succ, lift_val h]

@[simp] theorem WShape.lift_id {A a b : WShape n} (h : n ≤ m) :
    (WShape.id A a b).lift (m+1) = .id (A.lift m) (a.lift m) (b.lift m) := by
  ext; simp [lift_val (Nat.succ_le_succ h), Shape.lift, WShape.id, lift_val h]

@[simp] theorem WShape.lift_refl {w : WShape n} (h : n ≤ m) :
    (WShape.refl w : WShape (n+1)).lift (m+1) = WShape.refl (w.lift m) := by
  ext; simp [lift_val (Nat.succ_le_succ h), Shape.lift, WShape.refl, lift_val h]

theorem WShape.lift_pair' {a b : WShape n} (le : n ≤ m) :
    (WShape.pair' a b).lift (m+1) = .pair' (a.lift m) (b.lift m) := by
  ext1
  have hb_iff : ¬(a.lift m).val ≤ Shape.bot ∨ ¬(b.lift m).val ≤ Shape.bot ↔
      ¬a.val ≤ Shape.bot ∨ ¬b.val ≤ Shape.bot := by
    rw [lift_val le, lift_val le, not_congr (Shape.lift_le_bot le),
        not_congr (Shape.lift_le_bot le), Shape.le_bot, Shape.le_bot]
  simp only [pair']
  split <;> rename_i h
  · rw [dif_pos (hb_iff.2 h)]
    simp [pair, lift_val (Nat.succ_le_succ le), Shape.lift, lift_val le]
  · rw [dif_neg (mt hb_iff.1 h)]
    simp [bot, lift_val (Nat.succ_le_succ le), Shape.lift]
...
@[simp] theorem WShape.sigma_le_sigma {a a' : WShape n} {f f' : WShapeFun n} :
    WShape.sigma a f ≤ .sigma a' f' ↔ a ≤ a' ∧ f ≤ f' := by
  simp [LE.def, sigma, Shape.LE.def, WShapeFun.LE.def, WShape.LE.def]

@[simp] theorem WShape.pair_le_pair {a b a' b' : WShape n} {h h'} :
    WShape.pair a b h ≤ .pair a' b' h' ↔ a ≤ a' ∧ b ≤ b' := by
  simp [LE.def, pair, Shape.LE.def, WShape.LE.def]

theorem WShape.pair'_le_pair' {a b a' b' : WShape n} :
    WShape.pair' a b ≤ .pair' a' b' ↔ a ≤ a' ∧ b ≤ b' := by
  -- The smart constructor `pair'` collapses to bot iff both components ≤ bot.
  -- We characterise both sides via case analysis on the `if` guard.
  have aux {x y : WShape n} : (x.val ≤ .bot ∧ y.val ≤ .bot) ↔
      ¬(¬x.val ≤ Shape.bot ∨ ¬y.val ≤ Shape.bot) := by
    constructor
    · rintro ⟨h1, h2⟩ (h | h) <;> [exact h h1; exact h h2]
    · intro h
      refine ⟨?_, ?_⟩
      · by_contra hx; exact h (.inl hx)
      · by_contra hy; exact h (.inr hy)
  simp only [LE.def, pair']
  split <;> rename_i hL <;> split <;> rename_i hR
  · -- both pair (real)
    simp [pair, Shape.LE.def]
  · -- LHS pair (real), RHS bot — both sides False
    have ⟨hR1, hR2⟩ := aux.2 hR
    refine ⟨fun h => ?_, fun ⟨ha, hb⟩ => ?_⟩
    · simp [pair, bot, Shape.LE.def] at h
    · obtain hL | hL := hL
      · cases hL (ha.trans hR1)
      · cases hL (hb.trans hR2)
  · -- LHS bot, RHS pair (real)
    have ⟨hL1, hL2⟩ := aux.2 hL
    refine ⟨fun _ => ⟨hL1.trans Shape.bot_le, hL2.trans Shape.bot_le⟩, fun _ => ?_⟩
    simp [bot, pair, Shape.LE.def]
  · -- both bot
    have ⟨hL1, hL2⟩ := aux.2 hL
    refine ⟨fun _ => ⟨hL1.trans Shape.bot_le, hL2.trans Shape.bot_le⟩, fun _ => ?_⟩
    simp [bot, Shape.LE.def]
...
theorem WShape.le_forallE_iff {s : WShape (n+1)} {a' : WShape n} {f' : WShapeFun n} :
  ...
  · cases s using WShape.casesOn' with
    ...
    | _ => simp only [sort, lam, forallE, sigma, pair, unit,
      WShape.nat, WShape.zero, WShape.succ, WShape.id, refl, LE.def, Shape.LE.def, false_implies]
  · rintro (rfl | ⟨a, f, rfl, h1, h2⟩)
    · exact bot_le
    · exact forallE_le_forallE.2 ⟨h1, h2⟩

theorem WShape.le_sigma_iff {s : WShape (n+1)} {a' : WShape n} {f' : WShapeFun n} :
    s ≤ .sigma a' f' ↔ s = .bot ∨ ∃ a f, s = .sigma a f ∧ a ≤ a' ∧ f ≤ f' := by
  constructor
  · cases s using WShape.casesOn' with
    | bot => exact fun _ => .inl rfl
    | sigma a f => exact fun h => .inr ⟨a, f, rfl, sigma_le_sigma.1 h⟩
    | _ => simp only [sort, lam, forallE, sigma, pair, unit,
      WShape.nat, WShape.zero, WShape.succ, WShape.id, refl, LE.def, Shape.LE.def, false_implies]
  · rintro (rfl | ⟨a, f, rfl, h1, h2⟩)
    · exact bot_le
    · exact sigma_le_sigma.2 ⟨h1, h2⟩
...
theorem WShape.le_nat_iff {s : WShape (n+1)} :
    s ≤ WShape.nat ↔ s = .bot ∨ s = WShape.nat := by
  refine ⟨fun h => ?_, ?_⟩
  · cases s using WShape.casesOn' with
    | bot => exact .inl rfl
    | nat => exact .inr rfl
    | _ => simp [WShape.LE.def, WShape.nat, WShape.sort, WShape.lam, WShape.forallE,
      WShape.sigma, WShape.pair, WShape.zero, WShape.succ, WShape.unit, WShape.id, WShape.refl,
      Shape.LE.def, Shape.nat] at h
  · rintro (rfl | rfl)
    · exact bot_le
    · exact .rfl

theorem WShape.le_zero_iff {s : WShape (n+1)} :
    s ≤ WShape.zero ↔ s = .bot ∨ s = WShape.zero := by
  refine ⟨fun h => ?_, ?_⟩
  · cases s using WShape.casesOn' with
    | bot => exact .inl rfl
    | zero => exact .inr rfl
    | _ => simp [WShape.LE.def, WShape.zero, WShape.sort, WShape.lam, WShape.forallE,
      WShape.sigma, WShape.pair, WShape.nat, WShape.succ, WShape.unit, WShape.id, WShape.refl,
      Shape.LE.def, Shape.zero] at h
  · rintro (rfl | rfl)
    · exact bot_le
    · exact .rfl

theorem WShape.le_succ_iff {s : WShape (n+1)} {v' : WShape n} :
    s ≤ WShape.succ v' ↔ s = .bot ∨ ∃ v, s = .succ v ∧ v ≤ v' := by
  refine ⟨fun h => ?_, ?_⟩
  · cases s using WShape.casesOn' with
    | bot => exact .inl rfl
    | succ v => exact .inr ⟨v, rfl, (Shape.succ_le_succ).1 h⟩
    | _ => simp [WShape.LE.def, WShape.succ, WShape.sort, WShape.lam, WShape.forallE,
      WShape.sigma, WShape.pair, WShape.nat, WShape.zero, WShape.unit, WShape.id, WShape.refl,
      Shape.LE.def, Shape.succ] at h
  · rintro (rfl | ⟨v, rfl, h⟩)
    · exact bot_le
    · exact (Shape.succ_le_succ).2 h

theorem WShape.sort_le {s : WShape n} : .sort r ≤ s ↔ .sort r = s :=
  Shape.sort_le.trans <| by simp [WShape.ext_iff, WShape.sort]

theorem WShape.unit_le {s : WShape (n+1)} : .unit r ≤ s ↔ .unit r = s :=
  Shape.unit_le.trans <| by simp [WShape.ext_iff, WShape.unit]

theorem WShape.le_unit {s : WShape (n+1)} : s ≤ .unit r ↔ s = .bot ∨ s = .unit r :=
  Shape.le_unit.trans <| by simp [WShape.ext_iff, WShape.bot, WShape.unit]
...
theorem WShape.sigma_le {s : WShape (n+1)} {a : WShape n} {f : WShapeFun n} :
    WShape.sigma a f ≤ s ↔
      ∃ a' : WShape n, ∃ f' : WShapeFun n, a ≤ a' ∧ f ≤ f' ∧ s = .sigma a' f' := by
  constructor
  · intro h
    have ⟨a', b', h1, h2, h3⟩ := Shape.sigma_le.1 h
    have wf := h3 ▸ s.2
    exact ⟨⟨a', wf.1⟩, ⟨b', wf.2⟩, h1, h2, WShape.ext h3.symm⟩
  · intro ⟨a', f', h1, h2, h3⟩; subst h3; exact WShape.sigma_le_sigma.2 ⟨h1, h2⟩

@[simp] theorem WShape.id_le_id {A A' a a' b b' : WShape n} :
    WShape.id A a b ≤ .id A' a' b' ↔ A ≤ A' ∧ a ≤ a' ∧ b ≤ b' := by
  simp [WShape.LE.def, WShape.id, Shape.LE.def]

theorem WShape.id_le {s : WShape (n+1)} {A a b : WShape n} :
    WShape.id A a b ≤ s ↔
      ∃ A' a' b' : WShape n, A ≤ A' ∧ a ≤ a' ∧ b ≤ b' ∧ s = .id A' a' b' := by
  constructor
  · intro h
    have ⟨A', a', b', h1, h2, h3, h4⟩ := Shape.id_le.1 h
    have wf := h4 ▸ s.2
    exact ⟨⟨A', wf.1⟩, ⟨a', wf.2.1⟩, ⟨b', wf.2.2⟩, h1, h2, h3, WShape.ext h4.symm⟩
  · intro ⟨A', a', b', h1, h2, h3, h4⟩; subst h4
    exact WShape.id_le_id.2 ⟨h1, h2, h3⟩

theorem WShape.le_id_iff {s : WShape (n+1)} {A' a' b' : WShape n} :
    s ≤ .id A' a' b' ↔
      s = .bot ∨ ∃ A a b : WShape n, s = .id A a b ∧ A ≤ A' ∧ a ≤ a' ∧ b ≤ b' := by
  constructor
  · cases s using WShape.casesOn' with
    | bot => exact fun _ => .inl rfl
    | id A a b => exact fun h => .inr ⟨A, a, b, rfl, (WShape.id_le_id.1 h).1,
        (WShape.id_le_id.1 h).2.1, (WShape.id_le_id.1 h).2.2⟩
      | _ => simp only [sort, lam, forallE, sigma, pair, WShape.id, refl,
              WShape.nat, WShape.zero, WShape.succ, WShape.unit,
              LE.def, Shape.LE.def, false_implies]
  · rintro (rfl | ⟨A, a, b, rfl, h1, h2, h3⟩)
    · exact bot_le
    · exact WShape.id_le_id.2 ⟨h1, h2, h3⟩
...
@[simp] theorem WShape.Compat.unit_unit :
    Compat (.unit r : WShape (n+1)) (.unit r') ↔ r = r' := by
  simp [WShape.Compat, WShape.unit, Shape.Compat]
...
theorem WShape.join_prop {x y : WShape n} :
  ...
  · (cases x with | bot => exact fun _ _ _ _ => Shape.Compat.bot_l | _) <;>
    ...
    · exact ⟨(@ih ⟨_, wf.1⟩ ⟨_, wf'.1⟩).1 ⟨_, wf₃.1⟩ h2.1 h3.1,
        (go wf.2 wf'.2).1 ⟨_, wf₃.2⟩ h2.2 h3.2⟩
    · exact ⟨(@ih ⟨_, wf.1⟩ ⟨_, wf'.1⟩).1 ⟨_, wf₃.1⟩ h2.1 h3.1,
        (@ih ⟨_, wf.2.1⟩ ⟨_, wf'.2.1⟩).1 ⟨_, wf₃.2.1⟩ h2.2 h3.2⟩
    · exact (@ih ⟨_, wf⟩ ⟨_, wf'⟩).1 ⟨_, wf₃⟩ h2 h3
    · exact ⟨⟨(@ih ⟨_, wf.1⟩ ⟨_, wf'.1⟩).1 ⟨_, wf₃.1⟩ h2.1 h3.1,
        (@ih ⟨_, wf.2.1⟩ ⟨_, wf'.2.1⟩).1 ⟨_, wf₃.2.1⟩ h2.2.1 h3.2.1⟩,
        (@ih ⟨_, wf.2.2⟩ ⟨_, wf'.2.2⟩).1 ⟨_, wf₃.2.2⟩ h2.2.2 h3.2.2⟩
    · exact (@ih ⟨_, wf⟩ ⟨_, wf'⟩).1 ⟨_, wf₃⟩ h2 h3
  · (cases x with | bot => intro; exact ⟨wf', fun _ _ => (and_iff_right Shape.bot_le).symm⟩ | _) <;>
    ...
    simp +contextual [Shape.Compat, Shape.join, Shape.sort, Shape.nat, Shape.zero, Shape.succ,
      Shape.LE.def, Shape.WF]
    · intro h1 h2
    ...
    · intro h1 h2
      ...
      refine ⟨⟨a1, b1⟩, ?_⟩
      rintro ⟨⟨⟩⟩ <;> simp +contextual [Shape.WF, and_assoc, and_left_comm, *]
    -- pair: two component joins; disjunction premise from either side of input wf.
    · intro h1 h2
      have ⟨a1, a2⟩ := (@ih ⟨_, wf.1⟩ ⟨_, wf'.1⟩).2 h1
      have ⟨b1, b2⟩ := (@ih ⟨_, wf.2.1⟩ ⟨_, wf'.2.1⟩).2 h2
      simp only [WShape.LE.def] at a1 b1 a2 b2
      simp [WShape] at a2 b2 ⊢
      have lex_a : _ ≤ _ := ((a2 _ a1).1 .rfl).1
      have lex_b : _ ≤ _ := ((b2 _ b1).1 .rfl).1
      refine ⟨⟨a1, b1, ?_⟩, ?_⟩
      · obtain ha | hb := wf.2.2
        · exact .inl fun H => ha (lex_a.trans H)
        · exact .inr fun H => hb (lex_b.trans H)
      · intro a awf
        cases a with | pair s' f' => ?_ | _ => exact iff_of_false False.elim fun ⟨h, _⟩ => h.elim
        constructor
        · intro ⟨h1', h2'⟩
          have ⟨A1, A2⟩ := (a2 s' awf.1).1 h1'
          have ⟨B1, B2⟩ := (b2 f' awf.2.1).1 h2'
          exact ⟨⟨A1, B1⟩, A2, B2⟩
        · intro ⟨⟨A1, B1⟩, A2, B2⟩
          exact ⟨(a2 s' awf.1).2 ⟨A1, A2⟩, (b2 f' awf.2.1).2 ⟨B1, B2⟩⟩
    · intro h1
      have ⟨a1, a2⟩ := (@ih ⟨_, wf⟩ ⟨_, wf'⟩).2 h1
      simp only [WShape.LE.def] at a1 a2
      simp [WShape] at a2 ⊢
      refine ⟨a1, ?_⟩
      intro a awf
      cases a with | succ v' => ?_ | _ => exact iff_of_false False.elim fun ⟨h, _⟩ => h.elim
      exact a2 v' awf
    · intro h1 h2 h3
      have ⟨a1, a2⟩ := (@ih ⟨_, wf.1⟩ ⟨_, wf'.1⟩).2 h1
      have ⟨b1, b2⟩ := (@ih ⟨_, wf.2.1⟩ ⟨_, wf'.2.1⟩).2 h2
      have ⟨c1, c2⟩ := (@ih ⟨_, wf.2.2⟩ ⟨_, wf'.2.2⟩).2 h3
      simp only [WShape.LE.def] at a1 b1 c1 a2 b2 c2
      simp [WShape] at a2 b2 c2 ⊢
      refine ⟨⟨a1, b1, c1⟩, ?_⟩
      intro a awf
      cases a with | id A' a' b' => ?_  | _ => exact iff_of_false False.elim fun ⟨h, _⟩ => h.elim
      refine ⟨fun ⟨hA, ha, hb⟩ => ?_, fun ⟨⟨X1, Y1, Z1⟩, X2, Y2, Z2⟩ => ?_⟩
      · have ⟨X1, X2⟩ := (a2 A' awf.1).1 hA
        have ⟨Y1, Y2⟩ := (b2 a' awf.2.1).1 ha
        have ⟨Z1, Z2⟩ := (c2 b' awf.2.2).1 hb
        exact ⟨⟨X1, Y1, Z1⟩, X2, Y2, Z2⟩
      · exact ⟨(a2 A' awf.1).2 ⟨X1, X2⟩, (b2 a' awf.2.1).2 ⟨Y1, Y2⟩, (c2 b' awf.2.2).2 ⟨Z1, Z2⟩⟩
    · intro h1
      have ⟨a1, a2⟩ := (@ih ⟨_, wf⟩ ⟨_, wf'⟩).2 h1
      simp only [WShape.LE.def] at a1 a2
      simp [WShape] at a2 ⊢
      refine ⟨a1, ?_⟩
      intro a awf
      cases a with | refl w'' => ?_ | _ => exact iff_of_false False.elim fun ⟨h, _⟩ => h.elim
      exact a2 w'' awf
...
@[simp] theorem WShape.unit_join_unit :
    join (unit r : WShape (n+1)) (unit r') = if r = r' then .unit r else .bot := by
· rw [WShape.join]; split
  · ext1; simp [WShape.unit, Shape.join]; simp_all
  · simp_all
...
theorem Shape.Compat.sigma_sigma {a a' : Shape n} {f f' : ShapeFun n} :
    Compat (n := n+1) (.sigma a f) (.sigma a' f') ↔
    a.Compat a' ∧ ShapeFun.Compat Compat f f' := by simp only [Compat, Bool.and_eq_true]

@[simp] theorem WShape.Compat.sigma_sigma {a a' : WShape n} {f f' : WShapeFun n} :
    (WShape.sigma a f).Compat (.sigma a' f') ↔ a.Compat a' ∧ WShapeFun.Compat f f' :=
  Shape.Compat.sigma_sigma

@[simp] theorem WShape.sigma_join_sigma {a a' : WShape n} {f f' : WShapeFun n}
    (hc1 : a.Compat a') (hc2 : WShapeFun.Compat f f') :
    (WShape.sigma a f).join (.sigma a' f') = .sigma (a.join a') (f.join f') := by
...
theorem Shape.Compat.id_id {A A' a a' b b' : Shape n} :
    Compat (n := n+1) (.id A a b) (.id A' a' b') ↔
    A.Compat A' ∧ a.Compat a' ∧ b.Compat b' := by
  simp only [Compat, Bool.and_eq_true, and_assoc]

@[simp] theorem WShape.Compat.id_id {A A' a a' b b' : WShape n} :
    (WShape.id A a b).Compat (.id A' a' b') ↔ A.Compat A' ∧ a.Compat a' ∧ b.Compat b' :=
  Shape.Compat.id_id

@[simp] theorem WShape.id_join_id {A A' a a' b b' : WShape n}
    (hcA : A.Compat A') (hca : a.Compat a') (hcb : b.Compat b') :
    (WShape.id A a b).join (.id A' a' b') = .id (A.join A') (a.join a') (b.join b') := by
  have hc := Compat.id_id.2 ⟨hcA, hca, hcb⟩
  ext1; rw [join_val hc]
  simp [WShape.id, Shape.join, join_val hcA, join_val hca, join_val hcb]

@[simp] theorem WShape.refl_join_refl {w w' : WShape n} (hc : w.Compat w') :
    (WShape.refl w).join (WShape.refl w') = WShape.refl (w.join w') := by
  ext1; rw [join_val (show (WShape.refl w).Compat (WShape.refl w') from hc)]
  simp [WShape.refl, Shape.join, join_val hc]
...
theorem TShape.LE.sigma_decomp
    (le : (WShape.sigma (n := n) b f).T ≤ (WShape.sigma (n := n') b' f').T) :
    b.lift (max n n') ≤ b'.lift (max n n') ∧ f.lift (max n n') ≤ f'.lift (max n n') := by
  have le₁ := Nat.le_max_left n n'; have le₂ := Nat.le_max_right n n'
  have h := (TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)).1 le
  have h_raw : ((WShape.sigma b f).lift _).1 ≤ ((WShape.sigma b' f').lift _).1 := h
  rw [WShape.lift_val (Nat.succ_le_succ le₁), WShape.lift_val (Nat.succ_le_succ le₂)] at h_raw
  simp only [WShape.sigma, Shape.lift, Shape.LE.def] at h_raw
  constructor
  · show (b.lift _).1 ≤ (b'.lift _).1
    rw [WShape.lift_val le₁, WShape.lift_val le₂]; exact h_raw.1
  · show (f.lift _).1.LE (f'.lift _).1
...
theorem TShape.LE.pair'_decomp {a b : WShape n} {a' b' : WShape n'}
    (le : (WShape.pair' a b).T ≤ (WShape.pair' a' b').T) :
    a.lift (max n n') ≤ a'.lift (max n n') ∧ b.lift (max n n') ≤ b'.lift (max n n') := by
  have le₁ := Nat.le_max_left n n'; have le₂ := Nat.le_max_right n n'
  have h := (TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)).1 le
  rw [WShape.lift_pair' le₁, WShape.lift_pair' le₂] at h
  exact WShape.pair'_le_pair'.1 h

theorem TShape.LE.id_decomp {A a b : WShape n} {A' a' b' : WShape n'}
    (le : (WShape.id A a b).T ≤ (WShape.id A' a' b').T) :
    A.lift (max n n') ≤ A'.lift (max n n') ∧
    a.lift (max n n') ≤ a'.lift (max n n') ∧
    b.lift (max n n') ≤ b'.lift (max n n') := by
  have le₁ := Nat.le_max_left n n'; have le₂ := Nat.le_max_right n n'
  have h := (TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)).1 le
  rw [WShape.lift_id le₁, WShape.lift_id le₂] at h
  exact WShape.id_le_id.1 h
...
theorem TShape.succ_le_succ {v : WShape n} {v' : WShape n'} :
    (.succ v : WShape (n+1)).T ≤ (.succ v' : WShape (n'+1)).T ↔ v.T ≤ v'.T := by
  have le₁ := Nat.le_max_left n n'; have le₂ := Nat.le_max_right n n'
  refine (TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)).trans ?_
  rw [WShape.lift_succ le₁, WShape.lift_succ le₂, WShape.LE.def]
  exact .trans (by exact Shape.succ_le_succ) (TShape.LE.def le₁ le₂).symm

def TShape.bot : TShape := WShape.T (n := 0) .bot
def TShape.sort (r : Bool) : TShape := WShape.T (n := 0) (.sort r)
def TShape.type : TShape := .sort true
def TShape.unit (r : Bool) : TShape := WShape.T (n := 1) (.unit r)
...
theorem TShape.nat_eqv : (.nat : WShape (n+1)).T ≤ (.nat : WShape (n'+1)).T := by
  simp [TShape.LE.def']; rw [Nat.add_max_add_right]
  rw [WShape.lift_nat (Nat.le_max_left ..), WShape.lift_nat (Nat.le_max_right ..)]
  exact .rfl

theorem TShape.zero_eqv : (.zero : WShape (n+1)).T ≤ (.zero : WShape (n'+1)).T := by
  simp [TShape.LE.def']; rw [Nat.add_max_add_right]
  rw [WShape.lift_zero (Nat.le_max_left ..), WShape.lift_zero (Nat.le_max_right ..)]
  exact .rfl
...
theorem TShape.sigma_not_le_forallE {a : WShape n} {f : WShapeFun n}
    {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(.sigma a f : WShape (n+1)).T ≤ (.forallE a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_sigma le₁, WShape.lift_forallE le₂]
  simp [WShape.LE.def, WShape.sigma, WShape.forallE, Shape.LE.def]
...
theorem TShape.pair_not_le_forallE {a b : WShape n} {h}
    {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(.pair a b h : WShape (n+1)).T ≤ (.forallE a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_pair' le₁, WShape.lift_forallE le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.pair, WShape.forallE, Shape.LE.def] at hle
  · rename_i hcond
    simp [not_or] at hcond
    obtain hne | hne := h
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.1]; exact Shape.bot_le
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.2]; exact Shape.bot_le

theorem TShape.nat_not_le_forallE {a : WShape n'} {f : WShapeFun n'} :
    ¬(WShape.nat : WShape (n+1)).T ≤ (.forallE a f : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_forallE le₂]
  simp [WShape.LE.def, WShape.nat, WShape.forallE, Shape.LE.def, Shape.nat]

theorem TShape.zero_not_le_forallE {a : WShape n'} {f : WShapeFun n'} :
    ¬(WShape.zero : WShape (n+1)).T ≤ (.forallE a f : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_forallE le₂]
  simp [WShape.LE.def, WShape.zero, WShape.forallE, Shape.LE.def, Shape.zero]

theorem TShape.succ_not_le_forallE {v : WShape n} {a : WShape n'} {f : WShapeFun n'} :
    ¬(.succ v : WShape (n+1)).T ≤ (.forallE a f : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_forallE le₂]
  simp [WShape.LE.def, WShape.succ, WShape.forallE, Shape.LE.def, Shape.succ]

theorem TShape.unit_eqv :
    (WShape.unit (n := n) r).T ≤ .unit r ∧ .unit r ≤ (WShape.unit (n := n) r).T := by
  rw [TShape.LE.lift_r (Nat.le_add_left ..), TShape.LE.lift_l (Nat.le_add_left ..)]
  simp [unit, WShape.lift_unit, WShape.LE.rfl]
...
theorem TShape.sigma_not_le_lam' {a : WShape n} {f₁ : WShapeFun n} {f₂ : WShapeFun n'} :
    ¬(.sigma a f₁ : WShape (n+1)).T ≤ (WShape.lam' f₂).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_sigma le₁, WShape.lift_lam' le₂]
  intro hle; have ⟨_, _, _, _, hle⟩ := WShape.sigma_le.1 hle
  unfold WShape.lam' at hle; split at hle <;>
    simp [WShape.ext_iff, WShape.sigma, WShape.lam] at hle
  cases hle

theorem TShape.pair_not_le_lam' {a b : WShape n} {h} {f₂ : WShapeFun n'} :
    ¬(.pair a b h : WShape (n+1)).T ≤ (WShape.lam' f₂).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_pair' le₁, WShape.lift_lam' le₂]
  intro hle
  unfold WShape.pair' WShape.lam' at hle; split at hle
  · split at hle
    · simp [WShape.LE.def, WShape.pair, WShape.lam, Shape.LE.def] at hle
    · simp [WShape.LE.def, WShape.pair, WShape.bot, Shape.LE.def] at hle
  · rename_i hcond
    simp [not_or] at hcond
    obtain hne | hne := h
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.1]; exact Shape.bot_le
...
theorem TShape.nat_not_le_lam' {f : WShapeFun n'} :
    ¬(WShape.nat : WShape (n+1)).T ≤ (WShape.lam' f : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_lam' le₂]
  intro hle; unfold WShape.lam' at hle; split at hle <;>
    simp [WShape.LE.def, WShape.nat, WShape.lam, WShape.bot, Shape.LE.def, Shape.nat] at hle

theorem TShape.zero_not_le_lam' {f : WShapeFun n'} :
    ¬(WShape.zero : WShape (n+1)).T ≤ (WShape.lam' f : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_lam' le₂]
  intro hle; unfold WShape.lam' at hle; split at hle <;>
    simp [WShape.LE.def, WShape.zero, WShape.lam, WShape.bot, Shape.LE.def, Shape.zero] at hle

theorem TShape.succ_not_le_lam' {v : WShape n} {f : WShapeFun n'} :
    ¬(.succ v : WShape (n+1)).T ≤ (WShape.lam' f : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_lam' le₂]
  intro hle; unfold WShape.lam' at hle; split at hle <;>
    simp [WShape.LE.def, WShape.succ, WShape.lam, WShape.bot, Shape.LE.def, Shape.succ] at hle

theorem TShape.sort_not_le_sigma {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(.sort r : WShape n).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  rw [TShape.LE.def']; simp only [WShape.T, WShape.lift_sort]
  intro h; have h := congrArg (·.1) (WShape.sort_le.1 h)
  simp only [WShape.sort, WShape.sigma, WShape.lift_val (Nat.le_max_right ..)] at h
  have hk : max n (n' + 1) = max n (n' + 1) - 1 + 1 := by omega
  rw [hk] at h; simp [Shape.sort, Shape.lift] at h

theorem TShape.forallE_not_le_sigma {a : WShape n} {f : WShapeFun n}
    {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(.forallE a f : WShape (n+1)).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_forallE le₁, WShape.lift_sigma le₂]
  simp [WShape.LE.def, WShape.forallE, WShape.sigma, Shape.LE.def]

theorem TShape.lam_not_le_sigma {f₁ : WShapeFun n} {hl}
    {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(.lam f₁ hl : WShape (n+1)).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_lam le₁, WShape.lift_sigma le₂]
  simp [(· ≤ ·), Shape.LE, Shape.ble, WShape.lam, WShape.sigma]

theorem TShape.pair_not_le_sigma {a b : WShape n} {h}
    {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(.pair a b h : WShape (n+1)).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_pair' le₁, WShape.lift_sigma le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.pair, WShape.sigma, Shape.LE.def] at hle
  · rename_i hcond
    simp [not_or] at hcond
    obtain hne | hne := h
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.1]; exact Shape.bot_le
...
theorem TShape.unit_not_le_forallE :
    ¬(.unit r : WShape (n+1)).T ≤ (.forallE a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_forallE le₂]
  simp [WShape.LE.def, WShape.unit, WShape.forallE, Shape.LE.def]

theorem TShape.unit_not_le_lam' {f₂ : WShapeFun n'} :
    ¬(.unit r : WShape (n+1)).T ≤ (WShape.lam' f₂).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_lam' le₂]
  intro hle
  unfold WShape.lam' at hle; split at hle <;>
    simp [WShape.LE.def, WShape.unit, WShape.lam, WShape.bot, Shape.LE.def] at hle

theorem TShape.unit_not_le_sigma {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(.unit r : WShape (n+1)).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_sigma le₂]
  simp [WShape.LE.def, WShape.unit, WShape.sigma, Shape.LE.def]

theorem TShape.unit_not_le_pair' {a' b' : WShape n'} :
    ¬(.unit r : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle <;>
    simp [WShape.LE.def, WShape.unit, WShape.pair, WShape.bot, Shape.LE.def] at hle

theorem TShape.sort_not_le_unit :
    ¬(.sort r : WShape n).T ≤ (.unit r' : WShape (n'+1)).T := by
  rw [TShape.LE.def']; simp only [WShape.T, WShape.lift_sort]
  intro h; have h := congrArg (·.1) (WShape.sort_le.1 h)
  simp only [WShape.sort, WShape.unit, WShape.lift_val (Nat.le_max_right ..)] at h
  have hk : max n (n' + 1) = max n (n' + 1) - 1 + 1 := by omega
  rw [hk] at h; simp [Shape.sort, Shape.lift] at h

theorem TShape.forallE_not_le_unit {a : WShape n} {f : WShapeFun n} :
    ¬(.forallE a f : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_forallE le₁, WShape.lift_unit]
  simp [WShape.LE.def, WShape.forallE, WShape.unit, Shape.LE.def]

theorem TShape.sigma_not_le_unit {a : WShape n} {f : WShapeFun n} :
    ¬(.sigma a f : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_sigma le₁, WShape.lift_unit]
  simp [WShape.LE.def, WShape.sigma, WShape.unit, Shape.LE.def]

theorem TShape.lam_not_le_unit {f₁ : WShapeFun n} {hl} :
    ¬(.lam f₁ hl : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_lam le₁, WShape.lift_unit]
  simp [(· ≤ ·), Shape.LE, Shape.ble, WShape.lam, WShape.unit]

theorem TShape.pair_not_le_unit {a b : WShape n} {h} :
    ¬(.pair a b h : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_pair' le₁, WShape.lift_unit]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.pair, WShape.unit, Shape.LE.def] at hle
  · rename_i hcond
    simp [not_or] at hcond
    obtain hne | hne := h
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.1]; exact Shape.bot_le
...
theorem TShape.nat_not_le_unit :
    ¬(WShape.nat : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_unit]
  simp [WShape.LE.def, WShape.nat, WShape.unit, Shape.LE.def, Shape.nat]

theorem TShape.zero_not_le_unit :
    ¬(WShape.zero : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_unit]
  simp [WShape.LE.def, WShape.zero, WShape.unit, Shape.LE.def, Shape.zero]

theorem TShape.succ_not_le_unit {v : WShape n} :
    ¬(.succ v : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_unit]
  simp [WShape.LE.def, WShape.succ, WShape.unit, Shape.LE.def, Shape.succ]

theorem TShape.unit_not_le_nat :
    ¬(.unit r : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_nat le₂]
  simp [WShape.LE.def, WShape.unit, WShape.nat, Shape.LE.def, Shape.nat]

theorem TShape.unit_not_le_zero :
    ¬(.unit r : WShape (n+1)).T ≤ (WShape.zero : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_zero le₂]
  simp [WShape.LE.def, WShape.unit, WShape.zero, Shape.LE.def, Shape.zero]

theorem TShape.unit_not_le_succ {v : WShape n'} :
    ¬(.unit r : WShape (n+1)).T ≤ (.succ v : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_succ le₂]
  simp [WShape.LE.def, WShape.unit, WShape.succ, Shape.LE.def, Shape.succ]

theorem TShape.nat_not_le_sigma {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(WShape.nat : WShape (n+1)).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_sigma le₂]
  simp [WShape.LE.def, WShape.nat, WShape.sigma, Shape.LE.def, Shape.nat]

theorem TShape.zero_not_le_sigma {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(WShape.zero : WShape (n+1)).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_sigma le₂]
  simp [WShape.LE.def, WShape.zero, WShape.sigma, Shape.LE.def, Shape.zero]

theorem TShape.succ_not_le_sigma {v : WShape n} {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(.succ v : WShape (n+1)).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_sigma le₂]
  simp [WShape.LE.def, WShape.succ, WShape.sigma, Shape.LE.def, Shape.succ]

theorem TShape.sort_not_le_pair' {a' b' : WShape n'} :
    ¬(.sort r : WShape n).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  rw [TShape.LE.def']; simp only [WShape.T, WShape.lift_sort]
  intro h; have h := congrArg (·.1) (WShape.sort_le.1 h)
  simp only [WShape.sort, WShape.pair'] at h; split at h <;>
  · simp only [WShape.pair, WShape.bot, WShape.lift_val (Nat.le_max_right ..)] at h
    have hk : max n (n' + 1) = max n (n' + 1) - 1 + 1 := by omega
    rw [hk] at h; simp [Shape.sort, Shape.lift, Shape.bot] at h

theorem TShape.forallE_not_le_pair' {a : WShape n} {f : WShapeFun n} {a' b' : WShape n'} :
    ¬(.forallE a f : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_forallE le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.forallE, WShape.pair, Shape.LE.def] at hle
  · simp [WShape.LE.def, WShape.forallE, WShape.bot, Shape.LE.def] at hle

theorem TShape.sigma_not_le_pair' {a : WShape n} {f : WShapeFun n} {a' b' : WShape n'} :
    ¬(.sigma a f : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_sigma le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.sigma, WShape.pair, Shape.LE.def] at hle
  · simp [WShape.LE.def, WShape.sigma, WShape.bot, Shape.LE.def] at hle

theorem TShape.lam_not_le_pair' {f₁ : WShapeFun n} {hl} {a' b' : WShape n'} :
    ¬(.lam f₁ hl : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_lam le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.lam, WShape.pair, Shape.LE.def] at hle
  · simp [WShape.LE.def, WShape.lam, WShape.bot, Shape.LE.def] at hle

theorem TShape.nat_not_le_pair' {a' b' : WShape n'} :
    ¬(WShape.nat : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.nat, WShape.pair, Shape.LE.def, Shape.nat] at hle
  · simp [WShape.LE.def, WShape.nat, WShape.bot, Shape.LE.def, Shape.nat] at hle

theorem TShape.zero_not_le_pair' {a' b' : WShape n'} :
    ¬(WShape.zero : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.zero, WShape.pair, Shape.LE.def, Shape.zero] at hle
  · simp [WShape.LE.def, WShape.zero, WShape.bot, Shape.LE.def, Shape.zero] at hle

theorem TShape.succ_not_le_pair' {v : WShape n} {a' b' : WShape n'} :
    ¬(.succ v : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.succ, WShape.pair, Shape.LE.def, Shape.succ] at hle
  · simp [WShape.LE.def, WShape.succ, WShape.bot, Shape.LE.def, Shape.succ] at hle

theorem TShape.sort_not_le_nat :
    ¬(.sort r : WShape n).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ : n ≤ max n (n'+1) := Nat.le_max_left ..
  have' le₂ : n'+1 ≤ max n (n'+1) := Nat.le_max_right ..
  rw [TShape.LE.def le₁ le₂, WShape.lift_sort]
  cases hk : max n (n'+1) with
  | zero => intro h; have := Nat.eq_zero_of_le_zero (hk ▸ le₂); cases this
  | succ k =>
    have le₂' : n' ≤ k := by omega
    rw [WShape.lift_nat le₂']
    simp [WShape.LE.def, WShape.sort, WShape.nat, Shape.LE.def, Shape.nat]

theorem TShape.forallE_not_le_nat {a : WShape n} {f : WShapeFun n} :
    ¬(.forallE a f : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_forallE le₁, WShape.lift_nat le₂]
  simp [WShape.LE.def, WShape.forallE, WShape.nat, Shape.LE.def, Shape.nat]

theorem TShape.sigma_not_le_nat {a : WShape n} {f : WShapeFun n} :
    ¬(.sigma a f : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_sigma le₁, WShape.lift_nat le₂]
  simp [WShape.LE.def, WShape.sigma, WShape.nat, Shape.LE.def, Shape.nat]

theorem TShape.lam_not_le_nat {f : WShapeFun n} {hl} :
    ¬(.lam f hl : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_lam le₁, WShape.lift_nat le₂]
  simp [WShape.LE.def, WShape.lam, WShape.nat, Shape.LE.def, Shape.nat]

theorem TShape.pair_not_le_nat {a b : WShape n} {h} :
    ¬(.pair a b h : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_pair' le₁, WShape.lift_nat le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.pair, WShape.nat, Shape.LE.def, Shape.nat] at hle
  · rename_i hcond; simp [not_or] at hcond
    obtain hne | hne := h
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.1]; exact Shape.bot_le
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.2]; exact Shape.bot_le

theorem TShape.zero_not_le_nat {n n'} :
    ¬(WShape.zero : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_nat le₂]
  simp [WShape.LE.def, WShape.zero, WShape.nat, Shape.LE.def, Shape.zero, Shape.nat]

theorem TShape.succ_not_le_nat {v : WShape n} {n'} :
    ¬(WShape.succ v : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_nat le₂]
  simp [WShape.LE.def, WShape.succ, WShape.nat, Shape.LE.def, Shape.succ, Shape.nat]

theorem TShape.nat_not_le_zero :
    ¬(WShape.nat : WShape (n+1)).T ≤ (WShape.zero : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_zero le₂]
  simp [WShape.LE.def, WShape.nat, WShape.zero, Shape.LE.def, Shape.nat, Shape.zero]

theorem TShape.succ_not_le_zero {v : WShape n} :
    ¬(WShape.succ v : WShape (n+1)).T ≤ (WShape.zero : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_zero le₂]
  simp [WShape.LE.def, WShape.succ, WShape.zero, Shape.LE.def, Shape.succ, Shape.zero]

theorem TShape.forallE_not_le_succ {a : WShape n} {f : WShapeFun n} {v' : WShape n'} :
    ¬(.forallE a f : WShape (n+1)).T ≤ (WShape.succ v' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_forallE le₁, WShape.lift_succ le₂]
  simp [WShape.LE.def, WShape.forallE, WShape.succ, Shape.LE.def, Shape.succ]

theorem TShape.lam_not_le_succ {f : WShapeFun n} {hl} {v' : WShape n'} :
    ¬(.lam f hl : WShape (n+1)).T ≤ (WShape.succ v' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_lam le₁, WShape.lift_succ le₂]
  simp [WShape.LE.def, WShape.lam, WShape.succ, Shape.LE.def, Shape.succ]

theorem TShape.pair_not_le_succ {a b : WShape n} {h} {v' : WShape n'} :
    ¬(.pair a b h : WShape (n+1)).T ≤ (WShape.succ v' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_pair' le₁, WShape.lift_succ le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.pair, WShape.succ, Shape.LE.def, Shape.succ] at hle
  · rename_i hcond
    simp [not_or] at hcond
    obtain hne | hne := h
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.1]; exact Shape.bot_le
...
theorem TShape.nat_not_le_succ {v' : WShape n'} :
    ¬(WShape.nat : WShape (n+1)).T ≤ (WShape.succ v' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_succ le₂]
  simp [WShape.LE.def, WShape.nat, WShape.succ, Shape.LE.def, Shape.nat, Shape.succ]

theorem TShape.zero_not_le_succ {v' : WShape n'} :
    ¬(WShape.zero : WShape (n+1)).T ≤ (WShape.succ v' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_succ le₂]
  simp [WShape.LE.def, WShape.zero, WShape.succ, Shape.LE.def, Shape.zero, Shape.succ]

theorem TShape.succ_not_le_bot {v : WShape n} {n'} :
    ¬(WShape.succ v : WShape (n+1)).T ≤ (WShape.bot : WShape n').T := fun h => by
  have h1 : (WShape.succ v : WShape (n+1)).T ≤ TShape.bot := h.trans TShape.bot_eqv.1
  have h2 := TShape.le_bot.1 h1
  have h3 : (WShape.succ v : WShape (n+1)).1 = (WShape.bot : WShape (n+1)).1 :=
    congrArg (·.1) h2
  simp [WShape.succ, WShape.bot, Shape.succ, Shape.bot] at h3

theorem TShape.id_not_le_pair' {A a b : WShape n} {a' b' : WShape n'} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.id, WShape.pair, Shape.LE.def] at hle
  · simp [WShape.LE.def, WShape.id, WShape.bot, Shape.LE.def] at hle

theorem TShape.refl_not_le_pair' {w : WShape n} {a' b' : WShape n'} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (WShape.pair' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.refl, WShape.pair, Shape.LE.def, Shape.refl] at hle
  · simp [WShape.LE.def, WShape.refl, WShape.bot, Shape.LE.def, Shape.refl] at hle

theorem TShape.sort_not_le_refl {w : WShape n'} :
    ¬(.sort r : WShape n).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ : n ≤ max n (n'+1) := Nat.le_max_left ..
  have' le₂ : n'+1 ≤ max n (n'+1) := Nat.le_max_right ..
  rw [TShape.LE.def le₁ le₂, WShape.lift_sort]
  cases hk : max n (n'+1) with
  ...
  | succ k =>
    ...
    simp [WShape.LE.def, WShape.sort, WShape.refl, Shape.LE.def, Shape.refl]

theorem TShape.forallE_not_le_refl {a : WShape n} {f : WShapeFun n} {w : WShape n'} :
    ¬(.forallE a f : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_forallE le₁, WShape.lift_refl le₂]
  simp [WShape.LE.def, WShape.forallE, WShape.refl, Shape.LE.def, Shape.refl]

theorem TShape.lam_not_le_refl {f : WShapeFun n} {hl} {w : WShape n'} :
    ¬(.lam f hl : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_lam le₁, WShape.lift_refl le₂]
  simp [WShape.LE.def, WShape.lam, WShape.refl, Shape.LE.def, Shape.refl]

theorem TShape.sigma_not_le_refl {a : WShape n} {f : WShapeFun n} {w : WShape n'} :
    ¬(.sigma a f : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_sigma le₁, WShape.lift_refl le₂]
  simp [WShape.LE.def, WShape.sigma, WShape.refl, Shape.LE.def, Shape.refl]

theorem TShape.pair_not_le_refl {a b : WShape n} {h} {w : WShape n'} :
    ¬(.pair a b h : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_pair' le₁, WShape.lift_refl le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.pair, WShape.refl, Shape.LE.def, Shape.refl] at hle
  · rename_i hcond
    simp [not_or] at hcond
    obtain hne | hne := h
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.1]; exact Shape.bot_le
...
theorem TShape.id_not_le_refl {A a b : WShape n} {w : WShape n'} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_refl le₂]
  simp [WShape.LE.def, WShape.id, WShape.refl, Shape.LE.def, Shape.refl]

theorem TShape.refl_not_le_sort {w : WShape n} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (.sort r : WShape n').T := by
  have' le₁ : n+1 ≤ max (n+1) n' := Nat.le_max_left ..
  have' le₂ : n' ≤ max (n+1) n' := Nat.le_max_right ..
  rw [TShape.LE.def le₁ le₂, WShape.lift_sort]
  cases hk : max (n+1) n' with
  | zero => have := Nat.eq_zero_of_le_zero (hk ▸ le₁); cases this
  | succ k =>
    have le₁' : n ≤ k := by omega
    rw [WShape.lift_refl le₁']
    simp [WShape.LE.def, WShape.sort, WShape.refl, Shape.LE.def, Shape.refl]

theorem TShape.refl_not_le_forallE {w : WShape n} {a : WShape n'} {f : WShapeFun n'} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (.forallE a f : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_forallE le₂]
  simp [WShape.LE.def, WShape.refl, WShape.forallE, Shape.LE.def, Shape.refl]

theorem TShape.refl_not_le_lam {w : WShape n} {f : WShapeFun n'} {hl} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (.lam f hl : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_lam le₂]
  simp [WShape.LE.def, WShape.refl, WShape.lam, Shape.LE.def, Shape.refl]

theorem TShape.refl_not_le_sigma {w : WShape n} {a : WShape n'} {f : WShapeFun n'} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (.sigma a f : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_sigma le₂]
  simp [WShape.LE.def, WShape.refl, WShape.sigma, Shape.LE.def, Shape.refl]

theorem TShape.refl_not_le_pair {w : WShape n} {a b : WShape n'} {h} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (.pair a b h : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n'+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_refl le₁, WShape.lift_pair' le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.refl, WShape.pair, Shape.LE.def, Shape.refl] at hle
  · simp [WShape.LE.def, WShape.refl, WShape.bot, Shape.LE.def, Shape.refl] at hle

theorem TShape.refl_not_le_id {w : WShape n} {A a b : WShape n'} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (WShape.id A a b : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_id le₂]
  simp [WShape.LE.def, WShape.refl, WShape.id, Shape.LE.def, Shape.refl]

theorem TShape.sort_not_le_id {A' a' b' : WShape n'} :
    ¬(.sort r : WShape n).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  rw [TShape.LE.def']; simp only [WShape.T, WShape.lift_sort]
  intro h; have h := congrArg (·.1) (WShape.sort_le.1 h)
  simp only [WShape.sort, WShape.id, WShape.lift_val (Nat.le_max_right ..)] at h
  have hk : max n (n' + 1) = max n (n' + 1) - 1 + 1 := by omega
  rw [hk] at h; simp [Shape.sort, Shape.lift] at h

theorem TShape.lam_not_le_id {f : WShapeFun n} {hl} {A' a' b' : WShape n'} :
    ¬(.lam f hl : WShape (n+1)).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_lam le₁, WShape.lift_id le₂]
  simp [WShape.LE.def, WShape.lam, WShape.id, Shape.LE.def]

theorem TShape.pair_not_le_id {a b : WShape n} {h} {A' a' b' : WShape n'} :
    ¬(.pair a b h : WShape (n+1)).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂)]
  rw [show (WShape.pair a b h : WShape (n+1)) = WShape.pair' a b from
    WShape.pair_eq_pair']
  rw [WShape.lift_pair' le₁, WShape.lift_id le₂]
  intro hle
  unfold WShape.pair' at hle; split at hle
  · simp [WShape.LE.def, WShape.pair, WShape.id, Shape.LE.def] at hle
  · rename_i hcond
    simp [not_or] at hcond
    obtain hne | hne := h
    · apply hne; rw [(WShape.lift_le_bot le₁).1 hcond.1]; exact Shape.bot_le
...
theorem TShape.forallE_not_le_id {a : WShape n} {f : WShapeFun n} {A' a' b' : WShape n'} :
    ¬(.forallE a f : WShape (n+1)).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_forallE le₁, WShape.lift_id le₂]
  simp [WShape.LE.def, WShape.forallE, WShape.id, Shape.LE.def]

theorem TShape.sigma_not_le_id {a : WShape n} {f : WShapeFun n} {A' a' b' : WShape n'} :
    ¬(.sigma a f : WShape (n+1)).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_sigma le₁, WShape.lift_id le₂]
  simp [WShape.LE.def, WShape.sigma, WShape.id, Shape.LE.def]

theorem TShape.id_not_le_forallE {A a b : WShape n} {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (.forallE a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_forallE le₂]
  simp [WShape.LE.def, WShape.id, WShape.forallE, Shape.LE.def]

theorem TShape.id_not_le_sigma {A a b : WShape n} {a' : WShape n'} {f' : WShapeFun n'} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (.sigma a' f' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_sigma le₂]
  simp [WShape.LE.def, WShape.id, WShape.sigma, Shape.LE.def]

theorem TShape.id_not_le_lam' {A a b : WShape n} {f₂ : WShapeFun n'} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (WShape.lam' f₂).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_lam' le₂]
  intro hle
  unfold WShape.lam' at hle; split at hle
  · simp [WShape.LE.def, WShape.id, WShape.lam, Shape.LE.def] at hle
  · simp [WShape.LE.def, WShape.id, WShape.bot, Shape.LE.def] at hle

theorem TShape.refl_not_le_lam' {w : WShape n} {f₂ : WShapeFun n'} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (WShape.lam' f₂).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_lam' le₂]
  intro hle
  unfold WShape.lam' at hle; split at hle
  · simp [WShape.LE.def, WShape.refl, WShape.lam, Shape.LE.def, Shape.refl] at hle
  · simp [WShape.LE.def, WShape.refl, WShape.bot, Shape.LE.def, Shape.refl] at hle

theorem TShape.id_not_le_unit {A a b : WShape n} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_unit]
  simp [WShape.LE.def, WShape.id, WShape.unit, Shape.LE.def]

theorem TShape.refl_not_le_unit {w : WShape n} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (.unit r' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_unit]
  simp [WShape.LE.def, WShape.refl, WShape.unit, Shape.LE.def, Shape.refl]

theorem TShape.id_not_le_nat {A a b : WShape n} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_nat le₂]
  simp [WShape.LE.def, WShape.id, WShape.nat, Shape.LE.def, Shape.nat]

theorem TShape.refl_not_le_nat {w : WShape n} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (WShape.nat : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_nat le₂]
  simp [WShape.LE.def, WShape.refl, WShape.nat, Shape.LE.def, Shape.refl, Shape.nat]

theorem TShape.id_not_le_zero {A a b : WShape n} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (WShape.zero : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_zero le₂]
  simp [WShape.LE.def, WShape.id, WShape.zero, Shape.LE.def, Shape.zero]

theorem TShape.refl_not_le_zero {w : WShape n} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (WShape.zero : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_zero le₂]
  simp [WShape.LE.def, WShape.refl, WShape.zero, Shape.LE.def, Shape.refl, Shape.zero]

theorem TShape.unit_not_le_id {A' a' b' : WShape n'} :
    ¬(.unit r : WShape (n+1)).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_id le₂]
  simp [WShape.LE.def, WShape.unit, WShape.id, Shape.LE.def]

theorem TShape.nat_not_le_id {A' a' b' : WShape n'} :
    ¬(WShape.nat : WShape (n+1)).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_id le₂]
  simp [WShape.LE.def, WShape.nat, WShape.id, Shape.LE.def, Shape.nat]

theorem TShape.zero_not_le_id {A' a' b' : WShape n'} :
    ¬(WShape.zero : WShape (n+1)).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_id le₂]
  simp [WShape.LE.def, WShape.zero, WShape.id, Shape.LE.def, Shape.zero]

theorem TShape.succ_not_le_id {v : WShape n} {A' a' b' : WShape n'} :
    ¬(.succ v : WShape (n+1)).T ≤ (WShape.id A' a' b' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_id le₂]
  simp [WShape.LE.def, WShape.succ, WShape.id, Shape.LE.def, Shape.succ]

theorem TShape.unit_not_le_refl {w : WShape n'} :
    ¬(.unit r : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_unit, WShape.lift_refl le₂]
  simp [WShape.LE.def, WShape.unit, WShape.refl, Shape.LE.def, Shape.refl]

theorem TShape.nat_not_le_refl {w : WShape n'} :
    ¬(WShape.nat : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_nat le₁, WShape.lift_refl le₂]
  simp [WShape.LE.def, WShape.nat, WShape.refl, Shape.LE.def, Shape.nat, Shape.refl]

theorem TShape.zero_not_le_refl {w : WShape n'} :
    ¬(WShape.zero : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_refl le₂]
  simp [WShape.LE.def, WShape.zero, WShape.refl, Shape.LE.def, Shape.zero, Shape.refl]

theorem TShape.succ_not_le_refl {v : WShape n} {w : WShape n'} :
    ¬(.succ v : WShape (n+1)).T ≤ (WShape.refl w : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_refl le₂]
  simp [WShape.LE.def, WShape.succ, WShape.refl, Shape.LE.def, Shape.succ, Shape.refl]

theorem TShape.id_not_le_succ {A a b : WShape n} {v' : WShape n'} :
    ¬(WShape.id A a b : WShape (n+1)).T ≤ (WShape.succ v' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_id le₁, WShape.lift_succ le₂]
  simp [WShape.LE.def, WShape.id, WShape.succ, Shape.LE.def, Shape.succ]

theorem TShape.refl_not_le_succ {w : WShape n} {v' : WShape n'} :
    ¬(WShape.refl w : WShape (n+1)).T ≤ (WShape.succ v' : WShape (n'+1)).T := by
  have' le₁ := Nat.le_max_left ..; have' le₂ := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_refl le₁, WShape.lift_succ le₂]
  simp [WShape.LE.def, WShape.refl, WShape.succ, Shape.LE.def, Shape.refl, Shape.succ]
...
theorem TShape.zero_compat_succ_false {v' : WShape n'} :
    ¬(WShape.zero : WShape (n+1)).T.Compat (WShape.succ v' : WShape (n'+1)).T := by
  intro h
  have le₁ : n ≤ max n n' := Nat.le_max_left _ _
  have le₂ : n' ≤ max n n' := Nat.le_max_right _ _
  rw [TShape.Compat.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_zero le₁, WShape.lift_succ le₂] at h
  simp [WShape.Compat, WShape.zero, WShape.succ, Shape.Compat] at h

theorem TShape.succ_compat_zero_false {v : WShape n} :
    ¬(WShape.succ v : WShape (n+1)).T.Compat (WShape.zero : WShape (n'+1)).T := by
  intro h
  have le₁ : n ≤ max n n' := Nat.le_max_left _ _
  have le₂ : n' ≤ max n n' := Nat.le_max_right _ _
  rw [TShape.Compat.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_zero le₂] at h
  simp [WShape.Compat, WShape.zero, WShape.succ, Shape.Compat] at h

theorem TShape.succ_compat_succ_decomp {v : WShape n} {v' : WShape n'}
    (h : (WShape.succ v : WShape (n+1)).T.Compat (WShape.succ v' : WShape (n'+1)).T) :
    v.T.Compat v'.T := by
  have le₁ : n ≤ max n n' := Nat.le_max_left _ _
  have le₂ : n' ≤ max n n' := Nat.le_max_right _ _
  rw [TShape.Compat.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_succ le₂] at h
  have h_inner : WShape.Compat (v.lift (max n n')) (v'.lift (max n n')) := by
    simpa [WShape.Compat, WShape.succ, Shape.Compat] using h
  exact (TShape.Compat.def le₁ le₂).2 h_inner
...
theorem TShape.succ_le_succ_of_le {n n'} {a : WShape n} {b : WShape n'} (h : a.T ≤ b.T) :
    (WShape.succ a : WShape (n+1)).T ≤ (WShape.succ b : WShape (n'+1)).T := by
  have lm₁ := Nat.le_max_left n n'; have lm₂ := Nat.le_max_right n n'
  rw [TShape.LE.def (Nat.succ_le_succ lm₁) (Nat.succ_le_succ lm₂),
    WShape.lift_succ lm₁, WShape.lift_succ lm₂, WShape.LE.def]
  exact Shape.succ_le_succ.2 ((TShape.LE.def lm₁ lm₂).1 h)

theorem TShape.succ_compat_succ_of_compat {n n'} {v : WShape n} {v' : WShape n'}
    (h : v.T.Compat v'.T) :
    (WShape.succ v : WShape (n+1)).T.Compat (WShape.succ v' : WShape (n'+1)).T := by
  have le₁ := Nat.le_max_left n n'; have le₂ := Nat.le_max_right n n'
  rw [TShape.Compat.def (Nat.succ_le_succ le₁) (Nat.succ_le_succ le₂),
    WShape.lift_succ le₁, WShape.lift_succ le₂]
  simpa [WShape.Compat, WShape.succ, Shape.Compat] using
    (TShape.Compat.def le₁ le₂).1 h

theorem TShape.Join.succ_succ {n n'} {v : WShape n} {v' : WShape n'}
    (h : v.T.Compat v'.T) :
    TShape.Join (WShape.succ v : WShape (n+1)).T (WShape.succ v' : WShape (n'+1)).T
      (WShape.succ (v.T.join v'.T).2 : WShape ((v.T.join v'.T).1 + 1)).T := by
  have le₁ : n ≤ max n n' := Nat.le_max_left _ _
  have le₂ : n' ≤ max n n' := Nat.le_max_right _ _
  have hJ_pred := TShape.Join.mk h
  intro w
  refine ⟨fun hw =>
    ⟨(TShape.succ_le_succ_of_le hJ_pred.le.1).trans hw,
     (TShape.succ_le_succ_of_le hJ_pred.le.2).trans hw⟩, fun ⟨hw1, hw2⟩ => ?_⟩
  let K := max (max n n' + 1) w.1
  have hK1 : max n n' + 1 ≤ K := Nat.le_max_left _ _
  have h_w_K : w.1 ≤ K := Nat.le_max_right _ _
  obtain ⟨M', hK_eq⟩ : ∃ M', K = M' + 1 := ⟨K - 1, by omega⟩
  have hMax_M' : max n n' ≤ M' := by omega
  have h_n_M' : n ≤ M' := Nat.le_trans le₁ hMax_M'
  have h_n'_M' : n' ≤ M' := Nat.le_trans le₂ hMax_M'
  have h_j_M' : (v.T.join v'.T).1 ≤ M' := hMax_M'
  rw [TShape.LE.def (by omega : n + 1 ≤ K) h_w_K, hK_eq, WShape.lift_succ h_n_M'] at hw1
  rw [TShape.LE.def (by omega : n' + 1 ≤ K) h_w_K, hK_eq, WShape.lift_succ h_n'_M'] at hw2
  obtain ⟨w_pred_s, hw1_pred, hw_eq⟩ := Shape.succ_le.1 hw1
  have hw2_pred : (v'.lift M').1 ≤ w_pred_s := Shape.succ_le_succ.1 (hw_eq ▸ hw2)
  have hw_pred_wf : w_pred_s.WF := by
    have := (w.2.lift (M'+1)).2; rw [← hw_eq] at this; exact this
  have hCompat_M' : (v.lift M').Compat (v'.lift M') :=
    (TShape.Compat.def h_n_M' h_n'_M').1 h
  have h_join_le : (v.lift M').join (v'.lift M') ≤ (⟨w_pred_s, hw_pred_wf⟩ : WShape M') :=
    (WShape.Join.mk hCompat_M' _).mpr ⟨hw1_pred, hw2_pred⟩
  rw [TShape.LE.def hK1 h_w_K, hK_eq, WShape.lift_succ h_j_M']
  show Shape.LE (Shape.succ ((v.T.join v'.T).2.lift M').1) (w.2.lift (M'+1)).1
  rw [← hw_eq]
  refine Shape.succ_le_succ.2 ?_
  have hz_eq : ((v.T.join v'.T).2 : WShape (max n n')).lift M' = (v.lift M').join (v'.lift M') := by
    show ((v.lift (max n n')).join (v'.lift (max n n'))).lift M' = (v.lift M').join (v'.lift M')
    rw [WShape.lift_join hMax_M', WShape.lift_lift (.inl le₁), WShape.lift_lift (.inl le₂)]
  exact hz_eq ▸ h_join_le
...
/-- First projection on a pair shape; collapses to `.bot` on non-pair shapes. -/
def Shape.fst : Shape (n + 1) → Shape n
  | .pair a _ => a
  | _ => .bot

/-- Second projection on a pair shape; collapses to `.bot` on non-pair shapes. -/
def Shape.snd : Shape (n + 1) → Shape n
  | .pair _ b => b
  | _ => .bot

@[simp] theorem Shape.bot_fst : (@Shape.bot (n+1)).fst = .bot := rfl
@[simp] theorem Shape.bot_snd : (@Shape.bot (n+1)).snd = .bot := rfl
@[simp] theorem Shape.pair_fst {a b : Shape n} :
    Shape.fst (ShapeS.pair a b : Shape (n+1)) = a := rfl
@[simp] theorem Shape.pair_snd {a b : Shape n} :
    Shape.snd (ShapeS.pair a b : Shape (n+1)) = b := rfl

theorem Shape.lift_fst {s : Shape (n+1)} : lift m (fst s) = fst (lift (m+1) s) := by
  cases s <;> simp [fst, lift]

theorem Shape.lift_snd {s : Shape (n+1)} : lift m (snd s) = snd (lift (m+1) s) := by
  cases s <;> simp [snd, lift]

theorem Shape.fst_mono {s s' : Shape (n+1)} (h : s ≤ s') : fst s ≤ fst s' := by
  rw [LE.def] at h
  cases s <;> cases s' <;> simp_all [fst]

theorem Shape.snd_mono {s s' : Shape (n+1)} (h : s ≤ s') : snd s ≤ snd s' := by
  rw [LE.def] at h
  cases s <;> cases s' <;> simp_all [snd]

theorem Shape.fst_join {s s' : Shape (n+1)} (h : s.Compat s') :
    fst (s.join s') = (fst s).join (fst s') := by
  cases s <;> cases s' <;> simp_all [fst, join, Compat, bot_join, join_bot] <;> rfl

theorem Shape.snd_join {s s' : Shape (n+1)} (h : s.Compat s') :
    Shape.snd (s.join s') = (Shape.snd s).join (Shape.snd s') := by
  cases s <;> cases s' <;> simp_all [snd, join, Compat, bot_join, join_bot] <;> rfl

theorem Shape.Compat.fst {s s' : Shape (n+1)} (h : s.Compat s') :
    (Shape.fst s).Compat (Shape.fst s') := by
  cases s <;> cases s' <;> simp_all [Shape.fst, Compat] <;>
    first | exact Shape.Compat.bot_l | exact Shape.Compat.bot_r | exact h.1

theorem Shape.Compat.snd {s s' : Shape (n+1)} (h : s.Compat s') :
    (Shape.snd s).Compat (Shape.snd s') := by
  cases s <;> cases s' <;> simp_all [Shape.snd, Compat] <;>
    first | exact Shape.Compat.bot_l | exact Shape.Compat.bot_r | exact h.2
...
/-- First projection on a `WShape (n+1)`; collapses to `.bot` for non-pair shapes. -/
def WShape.fst (s : WShape (n+1)) : WShape n := by
  refine ⟨Shape.fst s.1, ?_⟩
  obtain ⟨⟨⟩, wf⟩ := s <;> first | exact .bot | exact wf.1

/-- Second projection on a `WShape (n+1)`; collapses to `.bot` for non-pair shapes. -/
def WShape.snd (s : WShape (n+1)) : WShape n := by
  refine ⟨Shape.snd s.1, ?_⟩
  obtain ⟨⟨⟩, wf⟩ := s <;> first | exact .bot | exact wf.2.1

@[simp] theorem WShape.bot_fst : fst (bot (n := n+1)) = .bot := rfl
@[simp] theorem WShape.bot_snd : snd (bot (n := n+1)) = .bot := rfl
@[simp] theorem WShape.pair_fst {a b : WShape n} {h} : fst (pair a b h) = a := rfl
@[simp] theorem WShape.pair_snd {a b : WShape n} {h} : snd (pair a b h) = b := rfl

theorem WShape.lift_fst {s : WShape (n+1)} (le : n ≤ m) :
    (fst s).lift m = fst (s.lift (m+1)) := by
  ext1; simp [lift_val le, fst, lift_val (Nat.succ_le_succ le), Shape.lift_fst]

theorem WShape.lift_snd {s : WShape (n+1)} (le : n ≤ m) :
    (snd s).lift m = snd (s.lift (m+1)) := by
  ext1; simp [lift_val le, snd, lift_val (Nat.succ_le_succ le), Shape.lift_snd]

theorem WShape.fst_mono {s s' : WShape (n+1)} (h : s ≤ s') : fst s ≤ fst s' := Shape.fst_mono h

theorem WShape.snd_mono {s s' : WShape (n+1)} (h : s ≤ s') : snd s ≤ snd s' := Shape.snd_mono h

theorem WShape.Compat.fst {s s' : WShape (n+1)} (h : s.Compat s') :
    (fst s).Compat (fst s') := Shape.Compat.fst h

theorem WShape.Compat.snd {s s' : WShape (n+1)} (h : s.Compat s') :
    (snd s).Compat (snd s') := Shape.Compat.snd h

theorem WShape.fst_join {s s' : WShape (n+1)} (h : s.Compat s') :
    WShape.fst (s.join s') = (WShape.fst s).join (WShape.fst s') := by
  ext1; show Shape.fst (s.join s').val = ((WShape.fst s).join (WShape.fst s')).val
  rw [WShape.join_val h, WShape.join_val (WShape.Compat.fst h)]
  exact Shape.fst_join h

theorem WShape.snd_join {s s' : WShape (n+1)} (h : s.Compat s') :
    WShape.snd (s.join s') = (WShape.snd s).join (WShape.snd s') := by
  ext1; show Shape.snd (s.join s').val = ((WShape.snd s).join (WShape.snd s')).val
  rw [WShape.join_val h, WShape.join_val (WShape.Compat.snd h)]
  exact Shape.snd_join h
...
theorem TShape.fst_mono {s : WShape (n + 1)} {s' : WShape (m + 1)} (le : s.T ≤ s'.T) :
    (WShape.fst s).T ≤ (WShape.fst s').T := by
  have lm₁ := Nat.le_max_left n m; have lm₂ := Nat.le_max_right n m
  rw [TShape.LE.def', WShape.lift_fst lm₁, WShape.lift_fst lm₂]
  refine WShape.fst_mono ?_
  exact (LE.def (Nat.succ_le_succ lm₁) (Nat.succ_le_succ lm₂)).1 le

theorem TShape.snd_mono {s : WShape (n + 1)} {s' : WShape (m + 1)} (le : s.T ≤ s'.T) :
    (WShape.snd s).T ≤ (WShape.snd s').T := by
  have lm₁ := Nat.le_max_left n m; have lm₂ := Nat.le_max_right n m
  rw [TShape.LE.def', WShape.lift_snd lm₁, WShape.lift_snd lm₂]
  refine WShape.snd_mono ?_
  exact (LE.def (Nat.succ_le_succ lm₁) (Nat.succ_le_succ lm₂)).1 le
...
def Shape.hasType : ∀ {n}, Shape n → Shape n → Bool
  ...
  | _+1, .bot, .unit _ => true
  | _+1, .unit rel, .sort j => rel ≤ j
  | _+1, .bot, .forallE a b => hasType.core hasType b a fun _ => .type
  | _+1, .forallE a b, .sort r => hasType.core hasType b a fun _ => .sort r
  | _+1, .bot, .sigma a b => hasType.core hasType b a fun _ => .type
  | _+1, .sigma a b, .sort true => hasType.core hasType b a fun _ => .type
  | _+1, .bot, .nat => true
  | _+1, .nat, .sort true => true
  | _+1, .bot, .id A a b => hasType a A && hasType b A
  | _+1, .id A a b, .sort true => hasType a A && hasType b A
  | _+1, .lam f, .forallE a b =>
    hasType.core hasType b a (fun _ => .type) && hasType.core hasType f a (ShapeFun.app b)
  | _+1, .pair x y, .sigma a b =>
    hasType.core hasType b a (fun _ => .type) && hasType x a && hasType y (ShapeFun.app b x)
  | _+1, .zero, .nat => true
  | n+1, .succ v, .nat => match n, v with
    | 0, .bot => true
    | _+1, v => hasType v .nat
    | _, _ => false
  | _+1, .refl w, .id A a b => hasType a A && hasType b A && hasType w A && w.ble a && w.ble b
  | _, _, _ => false
...
/-- A Σ-type signature is well-formed at `a` iff its codomain function has
domain `a` and lands in `Type`. (Σ doesn't preserve prop — always at `.sort true`.) -/
def Shape.HasTypeSigma (b : ShapeFun n) (a : Shape n) :=
  Shape.HasDom b a ∧ ∀ x y, (x, y) ∈ b → y.HasType .type

/-- A pair shape is well-typed at `(a, b)` iff `b` is a Σ codomain spec,
the fst component has type `a`, and the snd has type `b.app fst`. -/
def Shape.HasTypePair (x y : Shape n) (a : Shape n) (b : ShapeFun n) :=
  Shape.HasTypeSigma b a ∧ x.HasType a ∧ y.HasType (b.app x)

def Shape.HasTypeSucc : ∀ {n}, Shape n → Prop
  | 0, v => v = .bot
  | _+1, v => v.HasType .nat

/-- An Id-type signature is well-formed at `(A, a, b)` iff `A : Type` and
`a, b : A`. -/
def Shape.HasTypeId (A a b : Shape n) := a.HasType A ∧ b.HasType A

/-- A `refl` shape is well-typed at `.id A a b` iff `(A, a, b)` is a valid
Id-type signature. The equation `a ≡ b` that `.refl` semantically witnesses
lives at the IsDefEq level (via `reflDF`'s typing constraint), not at the
shape level — at the shape level any well-formed Id-type admits the refl
shape. -/
def Shape.HasTypeRefl (w A a b : Shape n) :=
  Shape.HasTypeId A a b ∧ w.HasType A ∧ w ≤ a ∧ w ≤ b
...
inductive Shape.HasTypeU : ∀ {n}, Shape n → Shape n → Prop
  ...
  | unit : r ≤ s → HasTypeU (n := n+1) (.unit r) (.sort s)
  | forallE : HasTypePi (n := n) b a r → HasTypeU (n := n+1) (.forallE a b) (.sort r)
  | lam : HasTypeLam (n := n) f a b → HasTypeU (n := n+1) (.lam f) (.forallE a b)
  | sigma : HasTypeSigma (n := n) b a → HasTypeU (n := n+1) (.sigma a b) .type
  | pair : HasTypePair (n := n) x y a b → HasTypeU (n := n+1) (.pair x y) (.sigma a b)
  | nat : HasTypeU (n := n+1) .nat .type
  | zero : HasTypeU (n := n+1) .zero .nat
  | succ {v : Shape n} : HasTypeSucc v → HasTypeU (n := n+1) (.succ v) .nat
  | id : HasTypeId (n := n) A a b → HasTypeU (n := n+1) (.id A a b) .type
  | refl : HasTypeRefl (n := n) w A a b → HasTypeU (n := n+1) (.refl w) (.id A a b)
...
theorem Shape.HasType.unfold {m a : Shape n} : HasType m a → HasTypeU m a := by
  ...
  · rfl
  · rfl
  · simp [HasType, hasType]
  · exact ‹_›
  · simp [HasType, hasType.core.iff, hasType]; exact ⟨‹_›, ‹_›⟩
  · simp [HasTypePi]; exact ⟨‹_›, ‹_›⟩
  · simp [HasType, hasType.core.iff, hasType]; exact ⟨‹_›, ‹_›⟩
  · simp [HasTypeSigma]; exact ⟨‹_›, ‹_›⟩
  · rfl
  · simp [HasType, hasType, *]
  · exact ⟨‹_›, ‹_›⟩
  · simp only [HasTypeLam, HasTypePi]; exact ⟨⟨‹_›, ‹_›⟩, ⟨‹_›, ‹_›⟩⟩
  · simp only [HasTypePair, HasTypeSigma]
    refine ⟨⟨‹_›, ‹_›⟩, ?_, ?_⟩ <;> assumption
  · rename_i n_v v h
    unfold HasTypeSucc; split at h <;> first | exact h | trivial
  · simp only [HasTypeRefl, HasTypeId]; exact ⟨⟨‹_›, ‹_›⟩, ‹_›, ‹_›, ‹_›⟩
...
theorem Shape.HasType.unfold_iff {m a : Shape n} : HasType m a ↔ HasTypeU m a := by
  ...
  | bot h =>
    ...
    | sigma => simpa [HasType, hasType] using h
    | id => simpa [HasType, hasType] using h
  | sort => cases n <;> rfl
  | unit => simpa [HasType, hasType]
  | forallE H => simpa [HasType, hasType, hasType.core.iff] using H
  | lam H => simp [HasType, hasType, hasType.core.iff]; exact H
  | sigma H => simpa [HasType, hasType, hasType.core.iff] using H
  | pair H =>
    simpa [HasType, hasType, hasType.core.iff, HasTypePair, HasTypeSigma, and_assoc] using H
  | nat => rfl
  | zero => rfl
  | @succ n v H => cases n <;> [(let .bot := v; rfl); exact H]
  | id H => simpa [HasType, hasType, HasTypeId] using H
  | refl H => simpa [HasType, hasType, HasTypeRefl, HasTypeId, and_assoc] using H
...
protected theorem Shape.HasType.lift (le : n ≤ n') :
  ...
  | succ n ih =>
    ...
    have eqt {s : Shape n} : (lift n' s).hasType (sort true) = s.hasType (sort true) := by
      rw [← lift_sort, ih]
    cases m <;> cases a <;> simp only [lift, hasType, type] <;> try rw [core lift_sort.symm]
    · rw [ih, ih]
    · rw [core (ShapeFun.lift_app le).symm]
    · rename_i rel; cases rel <;> simp [hasType] <;> rw [core lift_sort.symm]
    · rw [ih, ← ShapeFun.lift_app le, ih]
    · rename_i rel; cases rel <;> rfl
    · rename_i v
      cases n with | zero => cases v <;> cases n' <;> rfl | succ n_pp
      cases n' with | zero => cases le | succ n'
      exact ih (a := .nat)
    · rename_i rel; cases rel <;> simp only [hasType] <;> rw [ih, ih]
    · have eqb {s t : Shape n} : (lift n' s).ble (lift n' t) = s.ble t :=
        Bool.eq_iff_iff.2 (Shape.lift_le_lift le)
      simp only [ih, eqb]
...
protected theorem Shape.HasType.sigma (H : HasTypeSigma (n := n) b a) :
    HasType (n := n+1) (.sigma a b) .type := unfold_iff.2 (.sigma H)
protected theorem Shape.HasType.pair (H : HasTypePair (n := n) x y a b) :
    HasType (n := n+1) (.pair x y) (.sigma a b) := unfold_iff.2 (.pair H)
protected theorem Shape.HasType.nat : HasType (n := n+1) .nat .type := by
  show hasType (.nat : Shape (n+1)) (.sort true) = true; rfl
protected theorem Shape.HasType.zero : HasType (n := n+1) .zero .nat := by
  show hasType (.zero : Shape (n+1)) .nat = true; rfl
protected theorem Shape.HasType.succ {v : Shape n} (H : HasTypeSucc v) :
    HasType (n := n+1) (.succ v) .nat := by
  cases n with | zero => subst H; rfl | succ => exact H

/-- Eta: the only shape inhabiting `.unit r` is `.bot`. -/
theorem Shape.HasType.eq_bot_of_unit {m : Shape (n+1)} (H : HasType m (.unit r)) : m = .bot := by
  cases n <;> cases H.unfold <;> rfl

protected theorem Shape.HasType.id (H : HasTypeId (n := n) A a b) :
    HasType (n := n+1) (.id A a b) .type := unfold_iff.2 (.id H)
protected theorem Shape.HasType.refl (H : HasTypeRefl (n := n) w A a b) :
    HasType (n := n+1) (.refl w) (.id A a b) := unfold_iff.2 (.refl H)
...
theorem Shape.HasType.isType (H : HasType m a) : a.HasType .type := by
  cases H.unfold with
  | bot H => exact H
  | sort | forallE | sigma | nat | unit | id => exact .sort
  | lam H' => exact .forallE H'.1
  | pair H' => exact .sigma H'.1
  | zero | succ => exact .nat
  | refl H' => exact .id H'.1
...
def WShape.HasTypeSigma (b : WShapeFun n) (a : WShape n) := Shape.HasTypeSigma b.1 a.1
def WShape.HasTypePair (x y : WShape n) (a : WShape n) (b : WShapeFun n) :=
  Shape.HasTypePair x.1 y.1 a.1 b.1
def WShape.HasTypeId (A a b : WShape n) := Shape.HasTypeId A.1 a.1 b.1
def WShape.HasTypeRefl (w A a b : WShape n) := Shape.HasTypeRefl w.1 A.1 a.1 b.1
...
theorem WShape.HasTypeSigma.def {b : WShapeFun n} {a : WShape n} :
    HasTypeSigma b a ↔ HasDom b a ∧ ∀ x y, (x, y) ∈ b → y.HasType .type :=
  and_congr_right' ⟨fun H _ _ h => H _ _ h, fun H _ _ h => H _ _ (b.mem_val h)⟩

theorem WShape.HasTypePair.def {x y a : WShape n} {b : WShapeFun n} :
    HasTypePair x y a b ↔ HasTypeSigma b a ∧ x.HasType a ∧ y.HasType (b.app x) := .rfl

theorem WShape.HasTypeId.def : HasTypeId A a b ↔ a.HasType A ∧ b.HasType A := .rfl
...
theorem WShape.HasType.mono_r {m a a' : WShape n} (ha : a ≤ a')
  ...
  | sort | forallE | sigma | nat | unit | id => cases Shape.sort_le.1 ha; exact H
  | zero | succ => cases Shape.nat_le.1 ha; exact H
  | @refl n w A a_e b_e Hr =>
    obtain ⟨A', a_e', b_e', hA, hae, hbe, ⟨⟩⟩ := Shape.id_le.1 ha
    let .id Hid := Ha.unfold
    have ih := @WShape.HasType.mono_r n
    exact .refl ⟨Hid,
      ih (m := ⟨w, mwf⟩) (a := ⟨A, awf.1⟩) (a' := ⟨A', awf'.1⟩) hA Hid.1.isType Hr.2.1,
      Hr.2.2.1.trans hae, Hr.2.2.2.trans hbe⟩
  | @lam n _ _ _ H' =>
  ...
  | @pair n x y a_d b H' =>
    obtain ⟨_, _, h1, h2, ⟨⟩⟩ := Shape.sigma_le.1 ha
    let .sigma Ha_s := Ha.unfold
    have ih := @WShape.HasType.mono_r n
    let rec ih_pair {x y a a' : WShape n} {b b' : WShapeFun n} (Ha : HasTypeSigma b' a')
        (ha : a ≤ a') (hb : b ≤ b')
        (H : HasTypePair x y a b) : HasTypePair x y a' b' := by
      rw [HasTypePair.def] at H ⊢
      have ht_a : HasType a' .type := WShape.HasDom.isType (HasTypeSigma.def.1 Ha).1
      refine ⟨Ha, ih ha ht_a H.2.1, ?_⟩
      have ⟨_, hx1, hx2⟩ := b'.app_eq x
      exact .mono_r (WShapeFun.app_mono_l hb _)
        ((HasTypeSigma.def.1 Ha).2 _ _ hx2) H.2.2
    exact .pair <| ih_pair (x := ⟨x, mwf.1⟩) (y := ⟨y, mwf.2.1⟩)
        (a := ⟨_, awf.1⟩) (a' := ⟨_, awf'.1⟩) (b := ⟨_, awf.2⟩) (b' := ⟨_, awf'.2⟩) Ha_s h1 h2 H'
...
theorem ih_sigma {b b'} {a a' : WShape n}
    (hb1 : b ≤ b') (hb2 : b' ≤ b) (ha1 : a ≤ a') (ha2 : a' ≤ a)
    (H : HasTypeSigma b a) : HasTypeSigma b' a' := by
  rw [HasTypeSigma.def] at H ⊢
  refine ⟨ih_dom ih hb1 hb2 H.1 |>.mono_r ha1
    (ih ha1 ha2 (WShape.HasDom.isType H.1)), fun x y h => ?_⟩
  have ⟨x₁, y₁, a1, a2, a3⟩ := WShapeFun.LE.def'.1 hb2 _ _ h
  have ⟨x₂, y₂, b1, b2, b3⟩ := WShapeFun.LE.def'.1 hb1 _ _ a1
  have ⟨_, c1, c2⟩ := b.app_eq x
  exact ih (b3.trans <| b'.mem_mono b1 h (b2.trans a2)) a3 (H.2 _ _ a1)

theorem ih_pair {x x' y y' : WShape n} {a : WShape n} {b : WShapeFun n}
    (hx1 : x ≤ x') (hx2 : x' ≤ x) (hy1 : y ≤ y') (hy2 : y' ≤ y)
    (H : HasTypePair x y a b) : HasTypePair x' y' a b := by
  rw [HasTypePair.def] at H ⊢
  refine ⟨H.1, ih hx1 hx2 H.2.1, ?_⟩
  have ⟨_, _, c2⟩ := b.app_eq x'
  have hba_x' : (b.app x').HasType .type := (HasTypeSigma.def.1 H.1).2 _ _ c2
  exact .mono_r (WShapeFun.app_mono_r hx1) hba_x' (ih hy1 hy2 H.2.2)

theorem ih_id {A A' a a' b b' : WShape n}
    (hA1 : A ≤ A') (hA2 : A' ≤ A) (ha1 : a ≤ a') (ha2 : a' ≤ a)
    (hb1 : b ≤ b') (hb2 : b' ≤ b)
    (H : HasTypeId A a b) : HasTypeId A' a' b' := by
  obtain ⟨ha_t, hb_t⟩ := H
  have hA' : HasType A' (.sort true) := ih hA1 hA2 ha_t.isType
  refine ⟨?_, ?_⟩
  · exact WShape.HasType.mono_r hA1 hA' (ih ha1 ha2 ha_t)
  · exact WShape.HasType.mono_r hA1 hA' (ih hb1 hb2 hb_t)
...
theorem WShape.HasType.mono_l {m m' a : WShape n}
  ...
  | unit => cases Shape.unit_le.1 hm1; exact H
  | forallE H' =>
  ...
  | sigma H' =>
    obtain ⟨_, _, a1, a2, ⟨⟩⟩ := Shape.sigma_le.1 hm1
    have ⟨b1, b2⟩ := Shape.LE.def.1 hm2
    exact .sigma <| mono_l.ih_sigma mono_l (b := ⟨_, mwf.2⟩) (b' := ⟨_, mwf'.2⟩)
      (a := ⟨_, mwf.1⟩) (a' := ⟨_, mwf'.1⟩) a2 b2 a1 b1 H'
  | @pair n x y a_d b H' =>
    obtain ⟨_, _, a1, a2, ⟨⟩⟩ := Shape.pair_le.1 hm1
    have ⟨b1, b2⟩ := Shape.LE.def.1 hm2
    exact Shape.HasType.pair <| mono_l.ih_pair mono_l
      (x := ⟨_, mwf.1⟩) (x' := ⟨_, mwf'.1⟩) (y := ⟨_, mwf.2.1⟩) (y' := ⟨_, mwf'.2.1⟩)
      (a := ⟨_, awf.1⟩) (b := ⟨_, awf.2⟩) a1 b1 a2 b2 H'
  | nat => cases Shape.nat_le.1 hm1; exact Shape.HasType.nat
  | zero => cases Shape.zero_le.1 hm1; exact Shape.HasType.zero
  | @succ n_v v H' =>
    obtain ⟨v', hv1, ⟨⟩⟩ := Shape.succ_le.1 hm1
    have hv2 := Shape.succ_le_succ.1 hm2
    refine Shape.HasType.succ ?_
    cases n_v with | zero => cases H'; exact Shape.le_bot.1 hv2 | succ n'
    exact WShape.HasType.mono_l (n := n'+1) (m := ⟨v, mwf⟩)
      (m' := ⟨v', mwf'⟩) (a := ⟨.nat, trivial⟩) hv1 hv2 H'
  | @id n A a b H' =>
    obtain ⟨_, _, _, a1, a2, a3, ⟨⟩⟩ := Shape.id_le.1 hm1
    have ⟨b1, b2, b3⟩ := Shape.LE.def.1 hm2
    exact Shape.HasType.id <| mono_l.ih_id mono_l
      (A := ⟨_, mwf.1⟩) (A' := ⟨_, mwf'.1⟩)
      (a := ⟨_, mwf.2.1⟩) (a' := ⟨_, mwf'.2.1⟩)
      (b := ⟨_, mwf.2.2⟩) (b' := ⟨_, mwf'.2.2⟩) a1 b1 a2 b2 a3 b3 H'
  | @refl n_in w_in A_in a_in b_in Hr =>
    obtain ⟨w', rfl, hww⟩ := Shape.refl_le.1 hm1
    have hww2 := Shape.LE.def.1 hm2
    have ih := @WShape.HasType.mono_l n_in
    exact .refl ⟨Hr.1,
      ih (m := ⟨w_in, mwf⟩) (m' := ⟨w', mwf'⟩) (a := ⟨A_in, awf.1⟩) hww hww2 Hr.2.1,
      hww2.trans Hr.2.2.1, hww2.trans Hr.2.2.2⟩
...
/-- `HasTypeSigma` is `HasTypePi … true` (codomain at `.type = .sort true`),
so `lift`-stability follows from `HasTypePi.lift`. -/
theorem WShape.HasTypeSigma.lift (le : n ≤ m) :
    HasTypeSigma (b.lift m) (a.lift m) ↔ HasTypeSigma (n := n) b a := by
  show WShape.HasTypePi (b.lift m) (a.lift m) true ↔ WShape.HasTypePi b a true
  exact WShape.HasTypePi.lift le

def WShape.HasTypeSucc {n} (v : WShape n) : Prop := Shape.HasTypeSucc v.1

inductive WShape.HasTypeU : ∀ {n}, WShape n → WShape n → Prop
  | bot : HasType x .type → HasTypeU .bot x
  | sort : HasTypeU (.sort r) .type
  | unit : r ≤ s → HasTypeU (.unit r) (.sort s)
  | forallE : HasTypePi (n := n) b a r → HasTypeU (n := n+1) (.forallE a b) (.sort r)
  | lam : HasTypeLam (n := n) f a b → HasTypeU (n := n+1) (.lam' f) (.forallE a b)
  | sigma : HasTypeSigma (n := n) b a → HasTypeU (n := n+1) (.sigma a b) .type
  | pair {x y a : WShape n} {b : WShapeFun n} {h} :
    HasTypePair x y a b →
    HasTypeU (n := n+1) (.pair x y h) (.sigma a b)
  | nat : HasTypeU (n := n+1) WShape.nat WShape.type
  | zero : HasTypeU (n := n+1) WShape.zero WShape.nat
  | succ {v : WShape n} : HasTypeSucc v → HasTypeU (n := n+1) (.succ v) WShape.nat
  | id {A a b : WShape n} :
    HasTypeId A a b → HasTypeU (n := n+1) (.id A a b) .type
  | refl {w A a b : WShape n} :
    HasTypeRefl w A a b → HasTypeU (n := n+1) (.refl w) (.id A a b)
...
theorem WShape.HasType.unfold {m a : WShape n} (H : HasType m a) : HasTypeU m a := by
  ...
  | sigma h => exact .sigma (a := ⟨_, mwf.1⟩) (b := ⟨_, mwf.2⟩) h
  | pair h => exact .pair (x := ⟨_, mwf.1⟩) (y := ⟨_, mwf.2.1⟩) (h := mwf.2.2)
                      (a := ⟨_, awf.1⟩) (b := ⟨_, awf.2⟩) h
  | nat => exact .nat
  | zero => exact .zero
  | @succ _ v h => exact .succ (v := ⟨v, mwf⟩) h
  | id h => exact .id (A := ⟨_, mwf.1⟩) (a := ⟨_, mwf.2.1⟩) (b := ⟨_, mwf.2.2⟩) h
  | @refl _ w_in A_in a_in b_in h =>
    exact .refl (w := ⟨w_in, mwf⟩) (A := ⟨A_in, awf.1⟩)
      (a := ⟨a_in, awf.2.1⟩) (b := ⟨b_in, awf.2.2⟩) h
...
theorem WShape.HasType.unit : r ≤ s →
    HasType (n := n+1) (.unit r) (.sort s) := (unfold_iff.2 <| .unit ·)
...
theorem WShape.HasType.nat :
    HasType (n := n+1) WShape.nat WShape.type := unfold_iff.2 .nat
theorem WShape.HasType.zero :
    HasType (n := n+1) WShape.zero WShape.nat := unfold_iff.2 .zero
theorem WShape.HasType.succ {v : WShape n} (H : WShape.HasTypeSucc v) :
    HasType (n := n+1) (.succ v) WShape.nat := unfold_iff.2 (.succ H)

theorem WShape.HasType.nat_r {m : WShape (n+1)} (H : m.HasType WShape.nat) :
    m = .bot ∨ m = WShape.zero ∨ ∃ v : WShape n, m = WShape.succ v ∧ WShape.HasTypeSucc v := by
  cases H.unfold with
  | bot => exact .inl rfl
  | zero => exact .inr (.inl rfl)
  | succ H' => exact .inr (.inr ⟨_, rfl, H'⟩)
...
theorem WShape.HasType.refl_isType {w : WShape n} :
    ¬HasType (WShape.refl w : WShape (n+1)) (.sort r) := nofun

theorem WShape.HasType.refl_r {w : WShape n} {m : WShape (n+1)} :
    ¬HasType m (WShape.refl w : WShape (n+1)) := by
  intro h
  cases h.unfold with
  | bot h' => exact refl_isType h'

theorem WShape.refl_r {w : WShape n} {a' : WShape (n+1)}
    (le : (WShape.refl w : WShape (n+1)) ≤ a') (ht : a'.HasType .type) : False := by
  obtain ⟨w', hw', -⟩ := Shape.refl_le.1 (show (Shape.refl w.1 : Shape (n+1)) ≤ a'.1 from le)
  obtain ⟨a'_val, a'_wf⟩ := a'
  cases hw'
  exact WShape.HasType.refl_isType (n := n) (w := ⟨w', a'_wf⟩) ht

theorem WShape.refl_le_refl {n : Nat} {w w' : WShape n} :
    (WShape.refl w : WShape (n+1)) ≤ WShape.refl w' ↔ w ≤ w' := by
  constructor
  · intro h
    obtain ⟨w'', heq, hle⟩ := Shape.refl_le.1 (WShape.LE.def.1 h)
    obtain rfl : w'' = w'.1 := (ShapeS.refl.inj heq).symm
    exact WShape.LE.def.2 hle
  · intro h
    exact WShape.LE.def.2 (Shape.refl_le.2 ⟨w'.1, rfl, WShape.LE.def.1 h⟩)
...
theorem WShape.HasType.eq_bot_of_unit {x : WShape (n+1)} (H : HasType x (.unit r)) : x = .bot :=
  Subtype.ext (Shape.HasType.eq_bot_of_unit H)

theorem WShape.HasType.bot_iff : HasType (n := n) .bot x ↔ HasType x .type := ⟨.isType, .bot'⟩

/-- Monotonicity of `WShape.pair'` at TShape level: comparing two pair'
shapes (possibly at different levels) is determined by component-wise comparison. -/
theorem TShape.pair'_le_pair' {a : WShape n} {b : WShape n} {a' : WShape m} {b' : WShape m}
    (h1 : a.T ≤ a'.T) (h2 : b.T ≤ b'.T) :
    (a.pair' b).T ≤ (a'.pair' b').T := by
  have hk1 : n ≤ max n m := Nat.le_max_left ..
  have hk2 : m ≤ max n m := Nat.le_max_right ..
  rw [TShape.LE.def (Nat.succ_le_succ hk1) (Nat.succ_le_succ hk2)]
  rw [WShape.lift_pair' hk1, WShape.lift_pair' hk2]
  rw [WShape.pair'_le_pair']
  exact ⟨(TShape.LE.def hk1 hk2).1 h1, (TShape.LE.def hk1 hk2).1 h2⟩

/-- A value typed at a Σ-type must be `.bot` or `.pair _ _ h`. -/
theorem WShape.HasType.sigma_r {m : WShape (n+1)} {a : WShape n} {f : WShapeFun n}
    (H : m.HasType (.sigma a f)) :
    m = .bot ∨ ∃ x y h, m = .pair x y h ∧ HasTypePair x y a f := by
  generalize eq : WShape.sigma a f = t at H
  cases H.unfold with
  | bot _ => exact .inl rfl
  | sort | forallE | lam | sigma | nat | zero | succ | unit | id | refl => cases congrArg (·.1) eq
  | @pair _ x y a' b' h H' =>
    obtain ⟨rfl, rfl⟩ : a = a' ∧ f = b' := by
      cases a; cases a'; cases f; cases b'
      cases congrArg (·.1) eq; exact ⟨rfl, rfl⟩
    exact .inr ⟨x, y, h, rfl, H'⟩

/-- A value typed at an Id-type must be `.bot` or `.refl` with `HasTypeRefl`. -/
theorem WShape.HasType.id_r {m : WShape (n+1)} {A a b : WShape n}
    (H : m.HasType (.id A a b)) :
    m = .bot ∨ ∃ w, m = .refl w ∧ HasTypeRefl w A a b := by
  generalize eq : WShape.id A a b = t at H
  cases H.unfold with | bot _ => exact .inl rfl | @refl _ w' A' a' b' H' => ?_ | _ =>
    cases congrArg (·.1) eq
  obtain ⟨rfl, rfl, rfl⟩ : A = A' ∧ a = a' ∧ b = b' := by
    cases A; cases A'; cases a; cases a'; cases b; cases b'
    cases congrArg (·.1) eq; exact ⟨rfl, rfl, rfl⟩
  exact .inr ⟨w', rfl, H'⟩

theorem WShape.HasType.refl_iff :
    (WShape.refl w).HasType (WShape.id A a b) ↔ WShape.HasTypeRefl w A a b := by
  refine ⟨fun hm => ?_, .refl⟩
  obtain ⟨⟨⟩⟩ | ⟨w', hw', hR⟩ := WShape.HasType.id_r hm
  obtain rfl : w = w' := WShape.ext (ShapeS.refl.inj (congrArg (·.1) hw'))
  exact hR

theorem WShape.HasType.fst_proj {m : WShape (n+1)} {a : WShape n} {f : WShapeFun n}
    (ha : a.HasType .type) (H : m.HasType (.sigma a f)) : m.fst.HasType a := by
  generalize eq : WShape.sigma a f = t at H
  cases H.unfold with
  | bot _ => simp [WShape.bot_fst]; exact .bot' ha
  | sigma H' => cases congrArg (·.1) eq
  | sort | forallE | lam | nat | zero | succ | unit | id | refl => cases congrArg (·.1) eq
  | @pair _ x y a' b' h H' =>
    obtain ⟨rfl, rfl⟩ : a = a' ∧ f = b' := by
      cases a; cases a'; cases f; cases b'
      cases congrArg (·.1) eq; exact ⟨rfl, rfl⟩
...
theorem WShape.HasType.snd_proj {m : WShape (n+1)} {a : WShape n} {f : WShapeFun n}
    (hba : (f.app m.fst).HasType .type) (H : m.HasType (.sigma a f)) :
    m.snd.HasType (f.app m.fst) := by
  generalize eq : WShape.sigma a f = t at H
  cases H.unfold with
  | bot _ =>
    show (WShape.bot : WShape (n+1)).snd.HasType (f.app (WShape.bot : WShape (n+1)).fst)
    simp only [WShape.bot_snd, WShape.bot_fst]; exact .bot' hba
  | sigma H' => cases congrArg (·.1) eq
  | sort | forallE | lam | nat | zero | succ | unit | id | refl => cases congrArg (·.1) eq
  | @pair _ x y a' b' h H' =>
    obtain ⟨rfl, rfl⟩ : a = a' ∧ f = b' := by
      cases a; cases a'; cases f; cases b'
      cases congrArg (·.1) eq; exact ⟨rfl, rfl⟩
    show (WShape.pair x y h).snd.HasType (f.app (WShape.pair x y h).fst)
    simp only [WShape.pair_fst, WShape.pair_snd]
    exact H'.2.2
...
theorem WShape.HasTypePair.lift {x y a : WShape n} {b : WShapeFun n} (le : n ≤ m) :
    HasTypePair (x.lift m) (y.lift m) (a.lift m) (b.lift m) ↔
    HasTypePair (n := n) x y a b := by
  rw [HasTypePair.def, HasTypePair.def]
  refine and_congr (HasTypeSigma.lift le) (and_congr (WShape.HasType.lift le) ?_)
  rw [← WShapeFun.lift_app le]; exact WShape.HasType.lift le
...
theorem WShape.HasType.sigma_l {a : WShape n} {f : WShapeFun n} :
    HasType (.sigma a f) t ↔ HasTypeSigma f a ∧ t = .type := by
  simp only [HasType, WShape.sigma, HasTypeSigma, WShape.type, WShape.sort,
    WShape.ext_iff, Shape.HasType.unfold_iff]
  generalize a.1 = a₁, f.1 = f₁, t.1 = t₁
  refine ⟨fun (.sigma H) => ⟨H, rfl⟩, fun ⟨H, eq⟩ => eq ▸ .sigma H⟩

theorem WShape.HasType.id_l {A a b : WShape n} :
    HasType (WShape.id A a b) t ↔ HasTypeId A a b ∧ t = .type := by
  simp only [HasType, WShape.id, HasTypeId, WShape.type, WShape.sort,
    WShape.ext_iff, Shape.HasType.unfold_iff]
  generalize A.1 = A₁, a.1 = a₁, b.1 = b₁, t.1 = t₁
  refine ⟨fun (.id H) => ⟨H, rfl⟩, fun ⟨H, eq⟩ => eq ▸ .id H⟩
...
theorem WShape.HasType.join {m₁ m₂ a : WShape n} (hJ : m₁.Compat m₂)
  ...
  let rec go_pair {x₁ x₂ y₁ y₂ : WShape n} {a : WShape n} {b : WShapeFun n}
      (hx : x₁.Compat x₂) (hy : y₁.Compat y₂)
      (h1 : WShape.HasTypePair x₁ y₁ a b) (h2 : WShape.HasTypePair x₂ y₂ a b) :
      WShape.HasTypePair (x₁.join x₂) (y₁.join y₂) a b := by
    rw [WShape.HasTypePair.def] at h1 h2 ⊢
    refine ⟨h1.1, ih hx h1.2.1 h2.2.1, ?_⟩
    have hsig := WShape.HasTypeSigma.def.1 h1.1
    have h_jx := WShape.Join.mk hx
    have ⟨_, _, mj⟩ := b.app_eq (x₁.join x₂)
    have hba_xj : (b.app (x₁.join x₂)).HasType .type := hsig.2 _ _ mj
    have hy1' := hba_xj.mono_r (WShapeFun.app_mono_r h_jx.le.1) h1.2.2
    have hy2' := hba_xj.mono_r (WShapeFun.app_mono_r h_jx.le.2) h2.2.2
    exact ih hy hy1' hy2'
  let rec go_id {A₁ A₂ a₁ a₂ b₁ b₂ : WShape n}
      (hA : A₁.Compat A₂) (ha : a₁.Compat a₂) (hb : b₁.Compat b₂)
      (h1 : WShape.HasTypeId A₁ a₁ b₁) (h2 : WShape.HasTypeId A₂ a₂ b₂) :
      WShape.HasTypeId (A₁.join A₂) (a₁.join a₂) (b₁.join b₂) := by
    rw [WShape.HasTypeId.def] at h1 h2 ⊢
    have hjA := WShape.Join.mk hA
    have hA_T : (A₁.join A₂).HasType .type := ih hA h1.1.isType h2.1.isType
    refine ⟨?_, ?_⟩
    · exact ih ha (WShape.HasType.mono_r hjA.le.1 hA_T h1.1)
                  (WShape.HasType.mono_r hjA.le.2 hA_T h2.1)
    · exact ih hb (WShape.HasType.mono_r hjA.le.1 hA_T h1.2)
                  (WShape.HasType.mono_r hjA.le.2 hA_T h2.2)
  let go_refl {w₁ w₂ A a b : WShape n} (hw : w₁.Compat w₂)
      (h1 : WShape.HasTypeRefl w₁ A a b) (h2 : WShape.HasTypeRefl w₂ A a b) :
      WShape.HasTypeRefl (w₁.join w₂) A a b := by
    have hjw := WShape.Join.mk hw
    refine ⟨h1.1, ih hw h1.2.1 h2.2.1, ?_, ?_⟩
    · exact (hjw a).2 ⟨h1.2.2.1, h2.2.2.1⟩
    · exact (hjw b).2 ⟨h1.2.2.2, h2.2.2.2⟩
  ...
  | unit =>
    (cases m₂ with | bot => exact h1 | _) <;>
      simp only [Shape.Compat, decide_eq_true_eq, Bool.false_eq_true] at hJ
    subst hJ; simpa only [Shape.join, if_pos rfl]
  | forallE h1' =>
  ...
  | lam h1' =>
    ...
    exact .lam this
  | sigma h1' =>
    (cases h2.unfold with | bot => exact h1 | sigma h2' | _) <;>
      simp only [Shape.Compat, Bool.false_eq_true, Bool.and_eq_true] at hJ
    have := go_pi (b := ⟨_, wf₁.2⟩) (b' := ⟨_, wf₂.2⟩) (r := true)
      (a := ⟨_, wf₁.1⟩) (a' := ⟨_, wf₂.1⟩) hJ.1 hJ.2 h1' h2'
    rw [HasTypePi, WShape.join_val (by exact hJ.1), WShapeFun.join_val (by exact hJ.2)] at this
    exact .sigma this
  | pair h1' =>
    (cases h2.unfold with | bot => exact h1 | pair h2' | _) <;>
      simp only [Shape.Compat, Bool.and_eq_true] at hJ
    have := go_pair (x₁ := ⟨_, wf₁.1⟩) (x₂ := ⟨_, wf₂.1⟩)
      (y₁ := ⟨_, wf₁.2.1⟩) (y₂ := ⟨_, wf₂.2.1⟩)
      (a := ⟨_, wf'.1⟩) (b := ⟨_, wf'.2⟩) hJ.1 hJ.2 h1' h2'
    rw [HasTypePair, WShape.join_val (by exact hJ.1), WShape.join_val (by exact hJ.2)] at this
    exact .pair this
  | nat =>
    (cases h2.unfold with | bot => exact h1 | nat | _) <;>
      simp only [Shape.Compat, Bool.false_eq_true] at hJ
    exact h1
  | zero =>
    (cases h2.unfold with | bot => exact h1 | zero | _) <;>
      simp only [Shape.Compat, Bool.false_eq_true] at hJ
    exact h1
  | @succ _ v₁ h1' =>
    (cases h2.unfold with | bot => exact h1 | @succ _ v₂ h2' | _) <;>
      simp only [Shape.Compat, Bool.false_eq_true] at hJ
    refine Shape.HasType.succ ?_
    cases n with
    | zero => cases (h1' : v₁ = _); cases (h2' : v₂ = _); rfl
    | succ _ =>
      have hJ' : WShape.Compat (⟨v₁, wf₁⟩ : WShape _) ⟨v₂, wf₂⟩ := hJ
      have hj := ih (a := ⟨.nat, trivial⟩) hJ' h1' h2'
      rwa [WShape.HasType, WShape.join_val hJ'] at hj
  | id h1' =>
    (cases h2.unfold with | bot => exact h1 | id h2' | _) <;>
      simp only [Shape.Compat, Bool.false_eq_true, Bool.and_eq_true] at hJ
    have := go_id (A₁ := ⟨_, wf₁.1⟩) (A₂ := ⟨_, wf₂.1⟩)
      (a₁ := ⟨_, wf₁.2.1⟩) (a₂ := ⟨_, wf₂.2.1⟩)
      (b₁ := ⟨_, wf₁.2.2⟩) (b₂ := ⟨_, wf₂.2.2⟩) hJ.1.1 hJ.1.2 hJ.2 h1' h2'
    rw [HasTypeId, WShape.join_val (by exact hJ.1.1), WShape.join_val (by exact hJ.1.2),
        WShape.join_val (by exact hJ.2)] at this
    exact .id this
  | @refl _ w₁ A a_e b_e h1' =>
    (cases h2.unfold with | bot => exact h1 | refl h2' | _) <;>
      simp only [Shape.Compat] at hJ
    rename_i w₂
    have := go_refl (w₁ := ⟨w₁, wf₁⟩) (w₂ := ⟨w₂, wf₂⟩)
      (A := ⟨A, wf'.1⟩) (a := ⟨a_e, wf'.2.1⟩) (b := ⟨b_e, wf'.2.2⟩) hJ h1' h2'
    rw [HasTypeRefl, WShape.join_val (by exact hJ)] at this
    exact .refl this
...
theorem TShape.HasType.eq_bot_of_unit (H : HasType x (.unit r)) : x ≤ .bot := by
  have ⟨k, eq⟩ := Nat.exists_eq_add_of_le' (Nat.le_max_right x.1 1)
  simp [TShape.HasType, unit] at H; rw [eq, WShape.lift_unit] at H
  have h := WShape.HasType.eq_bot_of_unit H
  exact TShape.le_bot.2 ((WShape.lift_eq_bot (eq ▸ Nat.le_max_left x.1 1)).1 h)
...
theorem TShape.HasType.unit (h : r ≤ s) : HasType (.unit r) (.sort s) := by
  simp [HasType, TShape.unit, TShape.sort, WShape.lift_sort, WShape.HasType]
  exact WShape.HasType.unit h
...
theorem TShape.HasType.isType (H : HasType x y) : y.HasType .type := by
  have hk : y.1 ≤ max x.1 y.1 := Nat.le_max_right ..
  exact (TShape.HasType.def hk (Nat.zero_le _)).2
    ((TShape.HasType.def (Nat.le_max_left ..) hk).1 H).isType

inductive LE_Sigma {n} : TShape → WShape n → WShapeFun n → Prop where
  | bot : a ≤ .bot → LE_Sigma a b f
  | sigma : b'.T ≤ b.T → TShapeFun.LE (n := m) f' f →
    LE_Sigma (WShape.T (n := m+1) (.sigma b' f')) b f

theorem TShape.LE.le_sigma (ha : a ≤ WShape.T (n := n+1) (.sigma b f)) :
    LE_Sigma a b f := by
  ...
  | zero =>
    ...
    have hle_raw : (aw.lift _).1 ≤ ((WShape.sigma b f).lift _).1 := hle
    rw [WShape.lift_val (Nat.zero_le _), WShape.lift_val (Nat.le_refl _)] at hle_raw
    obtain ⟨val, wf⟩ := aw
    cases val with | bot => rfl | sort r
    simp [Shape.lift, WShape.sigma, Shape.LE.def] at hle_raw
  | succ m =>
    ...
    simp only [WShape.sigma, Shape.lift] at hle_raw
    obtain ⟨val, wf⟩ := aw
    cases val with
...
theorem TShape.HasType.toType (H : HasType x (.sort r)) : HasType x .type := by
  let k := x.1; have hk : x.1 ≤ k := Nat.le_refl _
  have h1 := (TShape.HasType.def hk (Nat.zero_le k)).1 H
  simp only [TShape.sort, WShape.lift_sort] at h1
  have h2 := h1.toType
  refine (TShape.HasType.def hk (Nat.zero_le k)).2 ?_
  simp only [TShape.type, TShape.sort, WShape.lift_sort]
  exact h2
...
theorem WShape.HasType.retype (ha : HasType (n := n) a (.sort r))
  ...
  | sigma =>
    obtain ⟨_, _, _, _, rfl⟩ := WShape.sigma_le.1 le
    obtain ⟨_, ⟨⟩⟩ := sigma_l.1 ha'
    exact ha
  | nat => cases show a' = WShape.nat from WShape.ext <| Shape.nat_le.1 le; exact ha'
  | id =>
    obtain ⟨_, _, _, _, _, _, rfl⟩ := WShape.id_le.1 le
    obtain ⟨_, ⟨⟩⟩ := id_l.1 ha'
    exact ha
